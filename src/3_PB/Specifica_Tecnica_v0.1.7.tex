...
Hook orchestratore dedicato alla gestione delle interazioni con l'Intelligenza Artificiale. Funge da regista tra la View, i servizi di rete e lo stato globale del widget, isolando la logica di invocazione della Facade \texttt{aiCall} e gestendo il ciclo di vita delle richieste asincrone tramite un \texttt{AbortController} mantenuto in un \texttt{React.RefObject} (\texttt{controllerRef}). È l'unico punto del sistema responsabile della creazione delle istanze delle classi di stato, nelle quali inietta le callback operative necessarie, e dell'invocazione di \texttt{changeState} sullo store \texttt{aiWidgetStore}.
...
\paragraph{Flusso Distant Writing}
L'operazione \textit{Distant Writing} richiede un prompt esplicito da parte dell'utente. Il suo flusso è gestito dalla funzione interna \texttt{handleDistantWriting}, che estende il percorso standard con uno stato iniziale di input e la possibilità di rigenerazione:

\begin{equation*}
\texttt{Idle} \to \texttt{Input} \to \texttt{Loading} \to \texttt{Done} \xrightarrow{\text{rigenera}} \texttt{Input}
\end{equation*}

La callback \texttt{onConfirm} è definita come \textit{closure} all'interno di \texttt{handleDistantWriting}: cattura il riferimento ad \texttt{onConfirm} stessa per permettere la rigenerazione con il medesimo prompt, passandola come quarto argomento al costruttore di \texttt{DoneState}.
...
La gestione dell'interfaccia utente per il widget AI adotta un'evoluzione del pattern \textit{State} classico, che può essere definita come \textbf{Render-Delegate State Pattern} (o \textit{UI State Object Pattern}). Questa scelta permette di gestire la complessa variabilità visiva del widget in modo modulare e polimorfico.

\subsubsection{Problema}
L'idea tradizionale dello \textit{State Pattern} (GoF) prevede che l'oggetto-stato modifichi esclusivamente il comportamento logico di un contesto, rimanendo agnostico rispetto alla presentazione visiva. In un ambiente dichiarativo come \gls{React}, applicare questo approccio costringerebbe il componente principale a gestire enormi blocchi condizionali (lunghi costrutti \texttt{switch} o \texttt{if/else}) per decidere quale porzione di interfaccia renderizzare, producendo un codice rigido, frammentato e difficile da testare.

\subsubsection{Soluzione: rendering delegato e stati polimorfi}
Nel sistema implementato, ogni stato non è solo comportamentale ma è anche \textbf{presentazionale}. Ogni oggetto-stato è direttamente responsabile del proprio rendering \gls{React}, permettendo al componente \texttt{AiWidget} di agire come un semplice coordinatore passivo.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{DoneState}}: Mostra il testo generato e le azioni disponibili: \textit{Inserisci} (inserisce il risultato nell'editor e torna a \texttt{IdleState}), \textit{Scarta} (ritorna a \texttt{IdleState}) e, opzionalmente, \textit{Rigenera} (riporta a \texttt{InputState} con il \gls{prompt} precedente, disponibile solo per l'operazione \textit{Distant Writing}).
...
Nel caso dell'operazione \textit{Distant Writing}, che richiede un \gls{prompt} esplicito dell'utente, il flusso viene esteso:
\begin{equation*}
\texttt{Idle} \to \texttt{Input} \to \texttt{Loading} \to \texttt{Done} \xrightarrow{\text{rigenera}} \texttt{Input}
\end{equation*}
...
Questa architettura garantisce una netta separazione tra rendering e logica: \texttt{AiWidget} è un componente puramente presentazionale che delega ogni decisione all'oggetto stato corrente; le classi di stato non contengono logica di business ma solo struttura visiva e callback iniettate; \texttt{useAiOperations} è l'unico punto in cui risiede la logica applicativa, le transizioni di stato e la gestione degli errori.
