...
Questo strumento è particolarmente utile per iniziare rapidamente a scrivere test, anche senza conoscere nel dettaglio le API di Playwright. Nel nostro caso, il codice generato è stato usato come base e poi migliorato aggiungendo controlli espliciti e una gestione più accurata delle operazioni asincrone.
...
In questo progetto sono stati realizzati quattro smoke test, uno per ciascuna funzionalità principale dell’editor: caricamento file, salvataggio/esportazione, stampa e operazioni AI. Ogni test esegue un flusso semplice (come farebbe un utente) e verifica con expect() che il risultato sia quello atteso.

Per quanto riguarda le funzionalità AI, le chiamate all’endpoint /api/ai/generate sono state simulate (mockate), così da rendere i test indipendenti dal backend e ottenere risultati sempre uguali e affidabili.

\subsubsection{load\_smoke.spec.ts}
\begin{itemize}
\item \textbf{Caricamento file}: questo test verifica che il pulsante “Carica File” funzioni correttamente. In particolare, controlla che al click venga aperto il dialog di selezione file del browser, segno che l’utente può caricare un documento.
\item \textbf{Stato}: Pass.
\end{itemize}

\subsubsection{save\_smoke.spec.ts}
\begin{itemize}
\item \textbf{Salvataggio ed esportazione file}: questo test verifica che la funzionalità di salvataggio ed esportazione funzioni correttamente. Dopo aver aperto il menu “Salva File”, viene selezionata l’opzione “Esporta” e si controlla che venga avviato il download di un file. Il test verifica inoltre che il file generato abbia un nome valido, assicurando che l’esportazione sia avvenuta con successo.
\item \textbf{Stato}: Pass.
\end{itemize}
