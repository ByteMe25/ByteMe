...
Questa sezione contiene i test specifici per la strategia di \gls{distant writing}. L'obiettivo è verificare che il sistema assembli correttamente il testo di contesto fornito dall'utente con il prompt personalizzato, garantendo che entrambi gli elementi siano presenti nell'output finale.
...
\begin{tabular}{|l|p{12cm}|c|}
    \hline
    \textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
    \prossimoUT & \gls{Verifica} che il \gls{Distant Writing} inserisca correttamente sia il \gls{prompt} utente sia il testo di contesto all'interno dei prompt generati verso il modello AI. & Pass \\ \hline
\end{tabular}
