...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        0.2.0 & 25/04/2026 & Elisa Marchioro & Aggiornamento subsubsection caching & \\
        \hline
        0.1.9 & 24/04/2026 & Matteo Cuogo & Aggiornamento dello stato di soddisfacimento di alcuni requisiti & \\
        \hline
        0.1.8 & 24/04/2026 & Tommaso Tombacco & Modifica sezione AiWidget e modifica dello stato dei requisiti legati ad AI & \\
        \hline
        0.1.7 & 23/04/2026 & Giulia Barzon & Aggiunte componenti AI, migliorata sezione 3 & \\
        \hline
        0.1.6 & 23/04/2026 & Elisa Marchioro & Sistemata formattazione, aggiunta sezione \gls{API} e aggiunta diagrammi UML backend & \\
        \hline
...
\begin{itemize}
\item Architettura client-server: separazione netta tra frontend e backend;
\item Statelessness: ogni richiesta contiene tutte le informazioni necessarie per essere elaborata; il server non mantiene stato tra richieste successive.
\item Interfaccia uniforme: utilizzo coerente di:
\begin{itemize}
...
DELETE → rimozione.
\end{itemize}
\item Formato dei dati standardizzato: scambio dati in formato JSON, facilmente leggibile e interoperabile tra sistemi eterogenei.
\item Separazione delle responsabilità (\gls{Separation of Concerns}): il client gestisce la presentazione, il server gestisce logica, orchestrazione e integrazione con servizi esterni.
\item Scalabilità: grazie alla natura \gls{stateless}, il backend può essere replicato e distribuito senza gestione di sessioni condivise.
\end{itemize}

\subsection{Comunicazione client–server (Axios)}
Nel frontend, la comunicazione con le \gls{API} REST avviene tramite la libreria Axios, un HTTP client basato su promise che semplifica l’invio di richieste asincrone e la gestione delle risposte. 
...
\begin{itemize}
\item Invio delle richieste al backend:
...
\item Gestione asincrona della comunicazione: permette al frontend di utilizzare async/await per gestire la chiamata senza bloccare l’interfaccia utente, migliorando la user experience.
\item Serializzazione automatica dei dati: converte automaticamente gli oggetti JavaScript in JSON per il body della richiesta, senza necessità di gestione manuale.
\item Gestione della risposta: riceve il JSON dal backend (es. generated\_text) e lo rende immediatamente disponibile al frontend per l’aggiornamento della UI.
\item Gestione degli errori: centralizza la gestione di errori HTTP o di rete (es. 400, 500), permettendo di definire comportamenti uniformi (messaggi utente, retry, logging).
\item Configurazione condivisa: consente di definire in un unico punto:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Persistence Logic}: Lo strato dedicato alla conservazione dei dati nel tempo.
\end{itemize}

% DIAGRAMMA COMPLETO DI TUTTA L'ARCHITETTURA (o dopo)
...
Permette l'astrazione della comunicazione con i provider esterni. Le classi contenute in questo file si occupano di iniettare le chiavi \gls{API} lette dall'ambiente e di impacchettare la richiesta nel formato corretto. Anche in questo caso le classi (\texttt{ZucchettiLlamaStrategy}, \texttt{Gemma3Strategy}) implementano il \textbf{Pattern Strategy}.
\begin{itemize}
    \item \textbf{Caching (\texttt{get\_model})}: All'interno di questo modulo è implementata una funzione di ottimizzazione che gestisce un dizionario globale privato (\texttt{\_model\_instances}). Se l'istanza di connessione a un modello non esiste, la crea; alle chiamate successive restituisce il riferimento in memoria, evitando di aprire connessioni ridondanti.
\end{itemize}


\subsubsection{Modulo: \texttt{text\_cleaning.py} (Servizi di supporto)}
...
% PERSISTENCE LOGIC
% ===================
\subsubsection{Persistence Logic}
Rappresenta lo strato inferiore deputato alla conservazione dei dati.  Coerentemente con la containerizzazione \gls{Docker}, l'architettura del backend è stata progettata per essere totalmente \textbf{\gls{stateless}} (priva di stato). Non è presente alcun \gls{database} relazionale lato server: la Persistence Logic per salvare i documenti dell'editor e la cronologia delle generazioni è demandata interamente allo storage locale del client, sfruttando le \gls{api} del \textbf{LocalStorage}. Questa architettura garantisce abbattimento del carico server, scalabilità orizzontale e \textit{Privacy by Design}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../UMLimg/backend.jpg}
    \caption{UML Backend}
...
\begin{itemize}
    ...
    \item \texttt{build(text: str)}: Metodo astratto che impone a ogni sottoclasse di restituire una tupla contenente due stringhe: il \gls{System prompt} (il ruolo dell'IA) e lo \gls{User prompt} (il task specifico).
\end{itemize}
\end{itemize}

\paragraph{SimplePromptStrategy}
Utilizzata per le operazioni editoriali standard (riassunto, correzione grammaticale, riscrittura, \gls{distant writing}). Mantiene la \gls{temperatura} di default a 0.5 e costruisce \gls{prompt} diretti impostando istruzioni ferree per evitare preamboli discorsivi.
\begin{itemize}
    \item\textbf{Attributi:}
\begin{itemize}
...
\end{itemize}
    \item\textbf{Metodi:}
\begin{itemize}
    \item \texttt{build(text)}: Combina \texttt{role} e \texttt{task} con il testo in ingresso per formare la tupla richiesta.
\end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../UMLimg/ai_strategy.jpg}
    \caption{UML Strategy AI}
...
\end{itemize}
    \item \textbf{Metodi:}
\begin{itemize}
    \item \texttt{model} (str): Stringa identificativa del modello instradato (es. \texttt{"llama3.2:3b"} o \texttt{"gemma3:4b"}).
...
Il meccanismo di caching è implementato tramite il decoratore \texttt{@lru\_cache} della libreria standard \texttt{functools}. Alla prima richiesta di un determinato modello, l’istanza viene creata e salvata; nelle chiamate successive, viene restituito direttamente il riferimento già memorizzato.

L’utilizzo di \texttt{functools} consente di affidarsi a una soluzione robusta e consolidata, che gestisce in modo trasparente anche gli aspetti legati alla concorrenza tra thread. Questo approccio riduce la complessità del codice e migliora l’affidabilità complessiva del sistema.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../UMLimg/model_strategy.jpg}
    \caption{UML Strategy ModelAI}
...
\paragraph{Flusso standard}
Per le operazioni che non richiedono input utente (riassunto, correzione, traduzioni, Sei Cappelli), il flusso di transizione è il seguente:

\begin{equation*}
\texttt{Idle} \xrightarrow{\text{handleAction}} \texttt{Loading} \xrightarrow{\text{successo}} \texttt{Done} \xrightarrow{\text{inserisci / scarta}} \texttt{Idle}
\end{equation*}

In caso di errore non derivante da cancellazione esplicita, il sistema transita in \texttt{ErrorState}; in caso di cancellazione (rilevata tramite \texttt{axios.isCancel}), la transizione viene soppressa e il widget rimane nell'ultimo stato coerente.

\paragraph{Flusso \gls{Distant Writing}}
L'operazione \textit{\gls{Distant Writing}} richiede un \gls{prompt} esplicito da parte dell'utente. Il suo flusso è gestito dalla funzione interna \texttt{handleDistantWriting}, che estende il percorso standard con uno stato iniziale di input e la possibilità di rigenerazione:

\begin{equation*}
\texttt{Idle} \to \texttt{Input} \to \texttt{Loading} \to \texttt{Done} \xrightarrow{\text{rigenera}} \texttt{Input}
\end{equation*}

La callback \texttt{onConfirm} è definita come \textit{closure} all'interno di \texttt{handleDistantWriting}: cattura il riferimento ad \texttt{onConfirm} stessa per permettere la rigenerazione con il medesimo \gls{prompt}, passandola come quarto argomento al costruttore di \texttt{DoneState}.
...
La progettazione del frontend di \textit{Second Brain} ha fatto uso di design pattern strutturali orientati alla semplificazione dell'interfaccia tra i moduli. L'obiettivo primario, applicando pattern come la Facade, è garantire che la logica di presentazione (View e ViewModel) rimanga agnostica rispetto ai dettagli implementativi della comunicazione di rete, facilitando la testabilità del codice client-side.

\subsection{Facade pattern}
Il pattern \textbf{Facade} è un design pattern strutturale il cui scopo principale è fornire un'interfaccia unica e semplificata per gestire un sottosistema complesso composto da molteplici moduli.
L'obiettivo è nascondere le dipendenze e i dettagli implementativi al client, fornendogli un singolo punto di accesso di alto livello dal quale effettuare le richieste. Il Facade riceve i comandi dal client e si occupa di associarli alle logiche specifiche del sottosistema, delegando loro l'effettiva elaborazione. Le classi interne implementano le funzionalità in modo del tutto indipendente, ignare dell'esistenza del Facade stesso.

L'applicazione di questo pattern comporta tre vantaggi architetturali primari:
\begin{itemize}
    \item \textbf{Accoppiamento debole (\textit{Loose coupling})}: Riduce drasticamente la superficie di contatto tra il client e il sottosistema, disaccoppiando chi utilizza i servizi da chi li implementa.
    \item \textbf{Semplificazione e stratificazione}: Diminuisce la complessità generale, elimina le dipendenze circolari e favorisce l'organizzazione del sistema in layer netti e impermeabili.
    \item \textbf{Manutenibilità e testabilità}: Isolare la complessità tecnica in un unico punto centralizzato facilita la scrittura di test automatici (tramite \textit{mocking}) e riduce l'impatto di eventuali refactoring.
\end{itemize}

\subsubsection{Applicazione del pattern: gestione della rete}
Nel sistema \textit{Second Brain}, questo pattern è stato applicato per la gestione delle comunicazioni HTTP con il backend. Il sistema implementa una Facade a due livelli sovrapposti, ciascuno con una responsabilità architetturale distinta. I ViewModel e i componenti della View non invocano mai direttamente la libreria esterna \texttt{Axios}; al contrario, si interfacciano con moduli dedicati che nascondono la complessità delle transazioni di rete dietro un'interfaccia unificata.

\paragraph{Livello 1: \texttt{apiClient.ts}}
Il primo livello costituisce la Facade sul sottosistema di rete. Non si limita ad avvolgere l'istanza di \texttt{Axios}, ma orchestra diverse componenti del networking: la configurazione dell'ambiente, la logica di intercettazione (\textit{Interceptor}), il sistema di formattazione degli errori (\texttt{transformApiError}) e il motore di cancellazione (\texttt{AbortController}). Fornisce così un'interfaccia unica e semplificata per tutte queste operazioni di basso livello. È responsabile di:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{ViewModel} (\texttt{useAiOperations}): Sa \textit{quando} chiamare l'AI e come gestire il risultato nella UI. Non conosce né \texttt{Axios} né l'URL dell'\gls{endpoint}.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../UMLimg/facade.jpg}
...
L’AI Widget è un componente che espone diverse funzionalità in base allo stato dell’operazione in corso. Ogni operazione attraversa una serie di fasi, rendendo esplicita all’utente la condizione della richiesta. Ad esempio, nel caso di un riassunto, il widget passa da uno stato di caricamento, che comunica la necessità di attendere e in cui è possibile annullare l'operazione, a uno stato di visualizzazione del risultato, che l’utente può inserire, eliminare o rigenerare.\\\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../UMLimg/widgetState.jpg}
...
\hline
RO-F-31 & Il sistema permette di inviare del testo ad un agente esterno. & Soddisfatto \\
\hline
RO-F-32 & L'utente deve poter accedere alle azioni sul testo tramite agente esterno in ogni momento. & Soddisfatto \\
\hline
RO-F-33 & Il testo generato viene prima mostrato come proposta, separata visivamente dal testo dell'utente. & Soddisfatto \\
\hline
RO-F-34 & Il testo in stato di proposta include le opzioni di conferma, annullamento e rigenerazione. & Soddisfatto \\
\hline
RO-F-35 & Il sistema non permette la modifica del testo generato mentre è in stato di proposta. & Soddisfatto \\
\hline
RO-F-36 & Il testo generato dall'agente esterno, una volta confermato, viene inserito nella posizione del cursore se presente, altrimenti in fondo al testo. & Soddisfatto \\
\hline
RO-F-37 & Una proposta che viene confermata viene inserita nel documento nell'editor e la sezione di proposta viene rimossa. & Soddisfatto \\
\hline
RO-F-38 & Una volta generato e confermato, il testo dell'agente esterno è modificabile tramite le stesse funzionalità dell'editor disponibili per il testo utente. & Soddisfatto \\
\hline
RO-F-39 & Una proposta annullata comporta la rimozione del testo generato dalla sezione di proposta e la rimozione della sezione stessa. & Soddisfatto \\
\hline
RO-F-40 & Una proposta che viene rigenerata viene rimandata all'agente esterno con gli stessi parametri e la nuova risposta sovrascrive la proposta corrente. & Soddisfatto \\
\hline
RO-F-41 & Il sistema deve informare l'utente con un messaggio in caso di errore durante la comunicazione con il servizio LLM. & Soddisfatto \\
\hline
RO-F-42 & L'utente può annullare un'operazione AI in corso prima del suo completamento. & Soddisfatto \\
\hline
RO-F-43 & In seguito all'annullamento di un'operazione AI, il documento rimane nello stato precedente alla richiesta. & Soddisfatto \\
\hline
RO-F-44 & Se la generazione di testo si blocca prematuramente, rimane ciò che è già stato generato, in attesa che l'utente confermi, rigeneri o elimini la proposta. & Soddisfatto \\
\hline
RO-F-45 & L'agente esterno applica modifiche esclusivamente al testo selezionato dall'utente. Un contesto aggiuntivo può essere fornito solo a scopo interpretativo e non deve essere alterato. & Soddisfatto \\
\hline
...
\hline
RO-F-48 & Ogni generazione di testo dell'agente esterno viene inclusa nel registro delle generazioni, incluse quelle annullate dall'utente. & Soddisfatto \\
\hline
...
\hline
RO-F-50 & L'utente deve poter riassumere tutto o una parte selezionata del testo usando un agente esterno. & Soddisfatto \\
\hline
RO-F-51 & L'utente deve poter usare un agente esterno per riscrivere tutto o una parte selezionata del testo migliorandolo seguendo determinate specifiche come sintassi, stile e chiarezza. & Soddisfatto \\
\hline
RO-F-52 & L'utente deve poter tradurre il testo usando un agente esterno in: inglese, italiano, tedesco, francese, spagnolo e cinese mandarino. & Soddisfatto \\
\hline
RO-F-53 & Se l'utente avvia una traduzione senza selezionare la lingua di destinazione, il sistema lo notifica e blocca l'operazione. & Soddisfatto \\
\hline
RO-F-54 & Il sistema permette di avviare la correzione grammaticale del testo tramite agente esterno. & Soddisfatto \\
\hline
RO-F-55 & Se non vengono rilevati errori grammaticali, il sistema informa l'utente dell'esito positivo della correzione. & Soddisfatto \\
\hline
RO-F-56 & Il sistema consente all'utente di avviare l'analisi critica del testo selezionato o dell'intero documento utilizzando il cappello di De Bono scelto. & Soddisfatto \\
\hline
RO-F-57 & Se il testo è insufficiente per l'analisi critica, il sistema notifica l'utente e interrompe l'operazione. & Soddisfatto \\
\hline
RO-F-58 & Se l'utente avvia l'analisi critica senza selezionare un cappello di De Bono, il sistema lo notifica e blocca l'operazione. & Soddisfatto \\
\hline
RO-F-59 & Il sistema permette l'invio di un \gls{prompt} scritto dall'utente per la generazione di testo tramite agente esterno. & Soddisfatto \\
\hline
RO-F-60 & Il sistema permette la modifica del \gls{prompt} da inviare all'agente esterno prima dell'invio. & Soddisfatto \\
\hline
RO-F-61 & L'agente esterno può generare testo basandosi su un \gls{prompt} fornitogli dall'utente. & Soddisfatto \\
\hline
RO-F-62 & Se il \gls{prompt} fornito dall'utente è vuoto o incompleto, il sistema notifica l'utente e impedisce l'invio. & Soddisfatto \\
\hline
...
\hline
RO-F-68 & Il sistema deve validare il formato del file selezionato e impedire il caricamento di formati non supportati. & Soddisfatto \\
\hline
RO-F-69 & Il sistema deve validare le dimensioni del file selezionato e impedire il caricamento di file che superano la dimensione massima consentita. & Soddisfatto \\
\hline
RO-F-70 & Il sistema deve impedire il caricamento di file corrotti o illeggibili. & Soddisfatto \\
\hline
RO-F-71 & Il sistema deve permettere all'utente di rimuovere il file che ha caricato. & Soddisfatto \\
\hline
RO-F-72 & L'utente può annullare l'operazione di rimozione del file caricato, lasciando l'editor nel suo stato precedente. & Soddisfatto \\
\hline
