...
\begin{itemize}
    ...
    \item \textbf{Esempio}: \texttt{POST /api/articles} crea un articolo; \texttt{DELETE /api/articles/42} lo elimina.
...
\begin{itemize}
    ...
    \item \textbf{Esempio}: \texttt{POST /api/generate} con body
...
A titolo comparativo, un'implementazione REST avrebbe richiesto endpoint separati per ogni operazione:
\begin{verbatim}
    POST /api/ai/summary
    POST /api/ai/rewrite
...
\begin{itemize}
    \item \textbf{Invio delle richieste al backend}: costruisce e invia la richiesta \texttt{POST} verso \texttt{/api/ai/generate} contenente i parametri dell'operazione, ovvero l'operazione richiesta dall'utente (\texttt{operationId}), il testo base (\texttt{text}) e l'eventuale istruzione aggiuntiva (\texttt{prompt}, utilizzato nel Distant Writing).
