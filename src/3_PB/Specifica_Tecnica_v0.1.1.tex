...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        0.1.1 & 21/04/2026 & Giulia Barzon & Struttura documento e sezione MVVM, bozza del Facade & \\
        \hline
        0.1.0 & 21/04/2026 & Giulia Barzon & Completate sezione 1, 2 e 3 & \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|p{0.10\textwidth}|p{0.55\textwidth}|}
        ...
        \hline
        React Router DOM & 7.13.1 & Libreria standard per il routing dichiarativo in applicazioni \gls{React}, utilizzata per la navigazione tra le viste. \\
        \hline
        ...
        \hline
        \gls{CSS Modules} & N/A (Integrato) & Sistema di scope locale per i fogli di stile CSS integrato in Vite, che evita conflitti tra classi. \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|p{0.10\textwidth}|p{0.55\textwidth}|}
        ...
        \hline
        React testing library & 16.3.2 & Libreria per il testing di componenti \gls{React}, che fornisce utilities per interagire con i componenti in modo simile a come un utente reale lo farebbe. \\
        \hline
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|p{0.10\textwidth}|p{0.55\textwidth}|}
        ...
        \hline
        Pydantic & 2.12.5 & Libreria per la validazione dei dati e la serializzazione tramite annotazioni di tipo nativo in Python. \\
        \hline
        Flask-CORS & 6.0.2 & Estensione di Flask per la gestione sicura del Cross-Origin Resource Sharing tra frontend e backend. \\
        \hline
        Python-dotenv & 1.2.2 & Modulo per il caricamento delle variabili d'ambiente e chiavi API dai file \texttt{.env}. \\
        \hline
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|p{0.10\textwidth}|p{0.55\textwidth}|}
        ...
        \textbf{Nome} & \textbf{Versione} & \textbf{Descrizione} \\
        \hline
        Nginx & alpine & Server web ad alte prestazioni utilizzato nel container \gls{Docker} per servire i file statici del frontend e gestire il routing. \\
        \hline
        Werkzeug & 3.1.8 & Server WSGI integrato in Flask, utilizzato attualmente come motore di esecuzione per l'ambiente backend. \\
        \hline
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|p{0.10\textwidth}|p{0.55\textwidth}|}
        ...
        \hline
        Docker Compose & 5.1.2 & Strumento per definire e gestire applicazioni multi-container tramite il file \texttt{docker-compose.yml}. \\
        \hline
    \end{tabular}
...
\begin{figure}[H]
    ...
    \caption{Rappresentazione dell'architettura di deployment tramite Docker Compose}
...
\paragraph{Props (Dati)}
\begin{itemize}
    \item \texttt{textAreaRef: React.RefObject<HTMLTextAreaElement | null>} - puntatore fisico al nodo DOM su cui istanziare la libreria esterna.
\end{itemize}
...
\paragraph{Props (Dati)}
\begin{itemize}
    \item \texttt{item: HistoryItem} - struttura dati completa dell'interazione (prompt, output, modello usato, data).
\end{itemize}
...
\begin{itemize}
    \item \texttt{textAreaRef: React.RefObject<HTMLTextAreaElement | null>} - riferimento iniettato nella view.
...
Gestisce il caricamento asincrono tramite \texttt{FileReader}. Esegue controlli preventivi (validazione del formato supportato \texttt{.md} o \texttt{.txt}, dimensione massima 5 MB) e previene la perdita accidentale di dati non salvati bloccando l'utente e chiedendogli conferma tramite \texttt{window.confirm}.

% grafico

\paragraph{Stato/Valori di ritorno}
\begin{itemize}
    \item \texttt{fileInputRef: React.RefObject<HTMLInputElement>} - riferimento fisico all'input nascosto di tipo \texttt{file} nel DOM.
\end{itemize}

\paragraph{Funzioni esposte}
\begin{itemize}
    \item \texttt{triggerFileInput: () => void} - simula il click sull'input nascosto per aprire la finestra di dialogo di sistema.
    \item \texttt{handleFileUpload: (event: React.ChangeEvent<HTMLInputElement>) => Promise<void>} - intercetta il file selezionato, lo valida e ne legge il contenuto testuale.
\end{itemize}
...
\paragraph{Funzioni esposte}
\begin{itemize}
    \item \texttt{handlePrint: (content: string) => void} - verifica la lunghezza del contenuto e innesca la stampa del browser o genera un errore visivo.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Configurazione centralizzata}: Risolve dinamicamente la \texttt{baseURL} dalle variabili d'ambiente (\texttt{VITE\_API\_BASE\_URL}), con \textit{fallback} a \texttt{/api}, e imposta il timeout (\texttt{VITE\_API\_TIMEOUT}, default 30.000 ms). Garantisce che un unico punto del codice controlli i parametri di connessione.
...
\paragraph{Livello 2: \texttt{aiCall.ts} (Astrazione dell'endpoint AI)}
Il secondo livello rappresenta una Facade sull'\textit{endpoint specifico} del backend. Nasconde l'URL esatto (\texttt{/ai/generate}) e la struttura del payload JSON, esponendo una singola funzione asincrona:
...
\begin{itemize}
    ...
    \item \textbf{ViewModel} (es. \texttt{useAiOperations}): Sanno \textit{quando} chiamare l'AI e come gestire il risultato nella UI. Non conoscono né \texttt{Axios} né l'URL dell'endpoint.
\end{itemize}
