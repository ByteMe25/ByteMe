...
\begin{itemize}
\item \textbf{ai\_strategies.py} - gestione dei \gls{prompt}: definisce come costruire i \gls{prompt} per ogni operazione. Ogni strategia incapsula le istruzioni e la \gls{temperatura} adatta al tipo di elaborazione richiesta, che sia un'operazione editoriale, una traduzione o un'analisi con i Sei Cappelli di De Bono.
\item \textbf{operation\_registry.py} - mappatura delle operazioni: contiene una mappatura statica che associa ogni identificatore di operazione alla strategia di \gls{prompt} e al modello LLM corretto tramite oggetti OperationConfig. Funge da punto di configurazione centrale del sistema.
\end{itemize}
...
I componenti principali sono:
\begin{itemize}
\item \textbf{model\_strategies.py} — integrazione modelli LLM: astrae la comunicazione con i provider esterni tramite l'interfaccia AIModelStrategy, esponendo i modelli \gls{Llama 3.2} e \gls{Gemma 3} in modo polimorfico. Gestisce l'iniezione delle variabili d'ambiente necessarie.
...
Questo file contiene la logica di costruzione dei \gls{prompt} tramite il pattern Strategy.

\paragraph{BaseAIStrategy} 
BaseAIStrategy è la classe astratta base del modulo. Dichiara la \gls{temperatura} di default, fornisce il metodo \_validate() per la \gls{validazione} dell'input e impone alle sottoclassi l'implementazione del metodo build(), che restituisce la coppia (system\_prompt, user\_prompt).
\begin{itemize}
...
\begin{itemize}
\item \_validate(): metodo concreto ereditabile che \gls{verifica} che il testo in ingresso non sia None, sollevando un TypeError in caso contrario.
...
SimplePromptStrategy è la strategia generica per operazioni editoriali standard. La \gls{temperatura} è 0.5.
\begin{itemize}

\item \textbf{Attributi}:

\begin{itemize}
\item role: stringa che definisce il ruolo assegnato al modello nel \gls{system \gls{prompt}}.
\item task: stringa che definisce l'istruzione specifica inserita nello \gls{user \gls{prompt}}.
\end{itemize}

\item \textbf{Metodi}: 

\begin{itemize}
\item build(): combina role e task con il testo in ingresso, costruendo un \gls{system \gls{prompt}} e \gls{user \gls{prompt}} con l'istruzione specifica.
\end{itemize}
...
TranslationStrategy è la strategia specializzata per le traduzioni. La \gls{temperatura} è 0.3, ridotta per favorire fedeltà al testo originale.
\begin{itemize}
...
\begin{itemize}
\item build(): costruisce un \gls{system \gls{prompt}} ottimizzato per la traduzione professionale e un \gls{user \gls{prompt}} che specifica la lingua e il testo da tradurre.
\end{itemize}
...
\begin{itemize}
\item \_system\_prompt: \gls{system \gls{prompt}} preconfigurato che definisce la prospettiva del cappello.
...
\begin{itemize}
\item build(): combina \_user\_task con il testo in ingresso nel \gls{user \gls{prompt}}, restituendo la coppia con il \gls{system \gls{prompt}} preconfigurato.
\end{itemize}

\end{itemize}

\paragraph{STRATEGIES}
STRATEGIES è il dizionario statico che raccoglie tutte le istanze delle strategie di \gls{prompt} disponibili nel sistema, indicizzate per identificatore di operazione (es. "summary", "black\_hat", "translate\_en"). Serve da sorgente da cui OPERATION\_REGISTRY attinge per comporre i propri OperationConfig.

\subsubsection{operation\_registry.py}
Questo file contiene la struttura dati OperationConfig, un dataclass che accoppia una strategia di \gls{prompt} con una classe di modelli AI. Il dizionario OPERATION\_REGISTRY costituisce la mappatura statica centrale del sistema: associa ogni identificatore di operazione (es. "summary", "black\_hat", "translate\_en") al corrispondente OperationConfig. Funge da punto di configurazione centrale, rendendo l'aggiunta di nuove operazioni una modifica localizzata.

\paragraph{OperationConfig}
Dataclass che accoppia una strategia di \gls{prompt} con una classe di modelli AI.
\begin{itemize}
\item model\_class: riferimento alla classe del modello LLM da istanziare per l'operazione.
\item \gls{prompt}\_strategy: istanza della strategia di \gls{prompt} da utilizzare per costruire i \gls{prompt} dell'operazione.
\end{itemize}

\subsubsection{model\_strategies.py}
Questo file permette l'astrazione della comunicazione con i provider esterni tramite il pattern Strategy. La classe base AIModelStrategy definisce l'interfaccia comune tramite il metodo generate(), che accetta i \gls{prompt} e la \gls{temperatura} e restituisce la risposta del modello.
...
ZucchettiLlamaStrategy è la strategia concreta per il modello \gls{Llama 3.2} 3B, utilizzato per le operazioni editoriali standard e le traduzioni.
\begin{itemize}

\item \textbf{Attributi}:

\begin{itemize}
\item client: istanza del client OpenAI-compatibile, inizializzata con \gls{API} key e base URL letti dalle variabili d'ambiente.
...
\begin{itemize}
\item generate(): invoca il modello tramite il client, passando \gls{system \gls{prompt}}, \gls{user \gls{prompt}} e \gls{temperatura}, e restituisce il contenuto testuale della risposta.
\end{itemize}

\end{itemize}

\paragraph{Gemma3Strategy} 
Gemma3Strategy è la strategia concreta per il modello \gls{Gemma 3} 4B, utilizzato per le analisi con i Sei Cappelli di De Bono. Strutturalmente identica a ZucchettiLlamaStrategy, si differenzia per l'identificativo del modello ("gemma3:4b").
\begin{itemize}
...
\begin{itemize}
\item app: istanza dell'applicazione \gls{Flask}, configurata con CORS abilitato per permettere la comunicazione col frontend.
\item generate\_ai\_text(): funzione principale del modulo, mappata sull'\gls{endpoint} POST /\gls{api}/ai/generate. Riceve il payload JSON, recupera la configurazione dalla mappatura OPERATION\_REGISTRY, istanzia il modello tramite get\_model(), costruisce i \gls{prompt} e restituisce la risposta elaborata. In caso di errore, restituisce una risposta standardizzata con il messaggio di eccezione.
\item root(): funzione mappata sull'\gls{endpoint} GET /, utilizzata come health check per verificare che il server sia attivo. Restituisce il nome del servizio, lo stato e la lista delle operazioni disponibili.
\end{itemize}
