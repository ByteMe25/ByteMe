...
\vspace{1.5cm}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{5cm}|}
            ...
            \hline
            \textbf{\gls{Verifica}} & Joseph Grant, Elisa Marchioro, Tommaso Tombacco \\
            \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        1.0.0 & 27/04/2026 & Giulia Barzon & Approvazione finale del documento & \\
        \hline
        0.1.3 & 26/04/2026 & Giulia Barzon & Inserito elenco delle figure & Joseph Grant, Elisa Marchioro\\
        \hline
        0.1.2 & 25/04/2026 & Giulia Barzon & Aggiunti screenshoot mancanti & Joseph Grant, Elisa Marchioro\\
        \hline
        0.1.1 & 24/04/2026 & Elisa Marchioro & Piccoli fix & Giulia Barzon, Joseph Grant\\
        \hline
        0.1.0 & 24/04/2026 & Giulia Barzon & Completate sezioni 1,2,3 e bozza di tutte le altre sezioni & Elisa Marchioro\\
        \hline
        0.0.1 & 01/04/2026 & Giulia Barzon & Creazione del documento, scrittura introduzione & Tommaso Tombacco, Elisa Marchioro \\
        \hline
    \end{tabular}
...
% Inserisce l'elenco delle figure
\phantomsection
...
\begin{itemize}
    ...
    \item \textbf{Spazio su disco:} Circa 2 GB di spazio libero per le immagini \gls{Docker}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Per utenti Windows/Mac}: Si consiglia l'installazione di \textit{\gls{Docker} Desktop}.
    \item \textbf{Per utenti Linux}: Assicurarsi che il servizio \texttt{\gls{docker}} sia attivo e che l'utente disponga dei permessi necessari per eseguire \texttt{\gls{docker}-compose}.
\end{itemize}
...
\subsection{Configurazione delle chiavi \gls{API}}
...
\begin{enumerate}
    ...
    \item Apri il file \texttt{.env} con un \gls{editor di testo} e compila i campi richiesti:
    \begin{itemize}
        ...
        \item \texttt{\gls{FLASK}\_SECRET\_KEY}: Inserisci una stringa casuale a tua scelta per mettere in sicurezza le sessioni tecniche del server.
    \end{itemize}
\end{enumerate}
...
\subsection{L'editor \gls{Markdown}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../ManualeUtenteimg/PulsantiToolbar.png}
    \caption{Legenda della barra degli strumenti dell'editor.}
    \label{fig:toolbar}
\end{figure}

\paragraph{\gls{Toolbar}: formattazioni \gls{Markdown}}
La barra degli strumenti permette di applicare i principali stili grafici senza dover digitare manualmente i simboli della sintassi \gls{Markdown}. Di seguito sono elencate le funzioni disponibili e la relativa sintassi che verrà inserita nel documento:

\begin{enumerate}
    \item \textbf{Undo e Redo delle operazioni}:
    \begin{itemize}
        \item \textbf{Undo (Annulla)}: Annulla l'ultima modifica effettuata all'interno dell'editor, permettendo di tornare immediatamente allo stato precedente del documento.
        \item \textbf{Redo (Ripristina)}: Riapplica un'operazione che è stata appena annullata tramite il comando Undo, recuperando la modifica scartata.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Sottolineato}: Sottolinea il testo selezionato.
    \end{itemize}
    \item \textbf{Intestazioni}:
    \begin{itemize}
        \item \textbf{Aumenta intestazione}: Incrementa il livello gerarchico del titolo (ad esempio da H2 a H1), aumentando la dimensione del carattere e l'importanza della sezione. Sintassi: aggiunge un simbolo \texttt{\#}.
        \item \textbf{Diminuisce intestazione}: Riduce il livello gerarchico del titolo (ad esempio da H1 a H2), diminuendo la dimensione del carattere per creare sottosezioni. Sintassi: rimuove un simbolo \texttt{\#}.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Linea orizzontale}: Inserisce un separatore di sezione grafico per dividere visivamente i contenuti. Sintassi: \texttt{---}.
        \item \textbf{Link}: Inserisce un collegamento ipertestuale su una porzione di testo selezionata. Sintassi: \texttt{[testo](url)}.
    \end{itemize}
    
    \item \textbf{Liste ed elenchi}:
    \begin{itemize}
    ...
    \item \textbf{Strumenti di modifica}:
    \begin{itemize}
        \item \textbf{Copia}: Memorizza la porzione di testo selezionata negli appunti di sistema per poterla riutilizzare.
        \item \textbf{Incolla}: Inserisce il testo precedentemente salvato negli appunti alla posizione attuale del cursore.
        \item \textbf{Taglia}: Rimuove il testo selezionato dal documento e lo memorizza negli appunti per spostarlo in un'altra posizione. 
    \end{itemize}

    \item \textbf{Modalità di visualizzazione}
    Per facilitare la revisione dei documenti, l'editor permette di alternare tra tre modalità di visualizzazione utilizzando le icone presenti nella \gls{toolbar} dell'area di testo:
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{\gls{Markdown} (.md)}: Mantiene il codice sorgente originale.
...
\begin{itemize}
    \item \textbf{Funzionamento}: Selezionando una lingua tra quelle supportate (Italiano, Inglese, Spagnolo, Francese, Tedesco), il sistema avvia una traduzione semantica.
...
\subsection{\gls{Distant Writing} (Generazione da zero)}
Il \textit{\gls{Distant Writing}} è lo strumento dedicato alla creazione di nuovi contenuti. 
\begin{itemize}
    \item \textbf{Processo}: Attivando questa funzione, comparirà una finestra di input dove l'utente può inserire un'istruzione (\gls{prompt}) descrivendo ciò che desidera ottenere (es: "Scrivi un'email formale di sollecito per una fattura scaduta").
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../ManualeUtenteimg/es_generazione.png}
    \caption{Esempio di widget che contiene il testo generato dall'AI. In basso a destra i pulsanti di gestione delle generazioni.}
    \label{fig:ai_widget_loading}
\end{figure}

\subsection{\gls{Distant Writing}: generazione creativa}
La funzione \textbf{\gls{Distant Writing}} è dedicata alla creazione di nuovi contenuti a partire da un'idea o un'istruzione specifica. A differenza degli strumenti editoriali, questa modalità richiede un input testuale diretto (\textit{\gls{prompt}}) da parte dell'utente.
...
\begin{enumerate}
    ...
    \item Apri il pannello AI e seleziona \textbf{\gls{Distant Writing}}.
    \item Scrivi un \gls{prompt}, un'istruzione o un'idea nel campo dedicato.
...
\begin{figure}[H]
    ...
    \caption{Interfaccia di input per il \gls{Distant Writing}.}
...
\begin{itemize}
    \item \textbf{Intestazione}: Mostra l'icona e il nome dell'operazione eseguita e il timestamp esatto (data e ora).
     \item \textbf{Testo iniziale}: Mostra il testo selezionato dall'utente, su cui è stata chiamata un'operazione di AI.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../ManualeUtenteimg/es_generazione2.png}
    \caption{Esempio di operazione AI: è stata fatta un'analisi del testo con il Cappello Bianco; l'utente ha confermato e inserito la risposta nell'editor e questa è stata salvata nello storico.}
    \label{fig:esempio_storico}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../ManualeUtenteimg/storico_generazione.png}
    \caption{Pulsanti per eliminare e copiare il testo di una generazione presente nello Storico.}
    \label{fig:elimina_generazione}
\end{figure}
...
\begin{itemize}
    \item \textbf{Soluzione}: Interrompi l'applicazione in conflitto, oppure apri il file \texttt{\gls{docker}-compose.yml} con un \gls{editor di testo} e modifica il parametro \texttt{ports} (es. da \texttt{"8080:8080"} a \texttt{"8081:8080"}). Successivamente, ripeti il comando di avvio e accedi a \texttt{http://localhost:8081}.
\end{itemize}

\paragraph{Il terminale mostra l'errore "\gls{docker}-compose command not found"}
...
\begin{itemize}
    \item \textbf{Soluzione}: Assicurati che l'applicazione \gls{Docker} Desktop sia aperta e in esecuzione in background prima di digitare i comandi nel terminale. Se utilizzi una versione molto recente, prova a digitare \texttt{\gls{docker compose}} (senza il trattino).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Soluzione 1}: \gls{Verifica} di aver creato correttamente il file \texttt{.env} nella cartella principale e di aver inserito chiavi \gls{API} valide, come spiegato nel capitolo di Installazione. Se hai modificato il file \texttt{.env} a sistema già avviato, devi riavviare l'applicazione (\texttt{Ctrl + C} nel terminale, seguito nuovamente da \texttt{\gls{docker}-compose up --build}).
