...
\begin{itemize}
   \item \textbf{Presentation Layer}: Gestione di UI/UX, dati e richieste tramite API REST;
...
È il livello più alto dell'architettura, fisicamente separato dal backend e delegato all'applicativo Frontend. Ha il compito di gestire l'interfaccia utente e la user experience, raccoglie gli input testuali e le selezioni dell'utente e invia le richieste al livello sottostante tramite API REST. Questo strato non esegue alcuna elaborazione semantica o logica di generazione, delegando l'intero carico computazionale al backend.

\subsection{Application Logic}
Rappresenta il punto di ingresso logico dell'applicazione lato backend. Nel progetto, questo livello è interamente implementato nel modulo di routing del framework Flask all'interno del file app.py. 
...
L'Application Logic si occupa esclusivamente del routing e dell'orchestrazione: non possiede alcuna conoscenza diretta delle regole di dominio, della struttura dei prompt o delle librerie di rete utilizzate per contattare l'Intelligenza Artificiale.


\subsection{Business Logic}
È il nucleo applicativo del sistema, dove risiedono le regole funzionali di Second Brain. Questo strato è rigidamente isolato dall'Application Logic: non possiede alcuna conoscenza del framework web (HTTP, Flask) né di come i dati verranno presentati o salvati, garantendo così il massimo disaccoppiamento. Al suo interno fa uso di pattern comportamentali (Strategy) per gestire la variabilità degli algoritmi in modo pulito ed estensibile.

I componenti principali di questo livello si dividono in servizi di dominio e servizi di integrazione:
\begin{itemize}
    \item Gestione dei Prompt;
    \item Orchestrazione delle Operazioni;
    \item Integrazione Modelli LLM;
    \item Servizi di Supporto al Dominio.
\end{itemize}

\subsubsection{Gestione dei Prompt (ai\_strategies.py)}
...
Implementa il pattern Registry. Contiene le strutture dati (OperationConfig) che mappano dinamicamente gli identificatori delle richieste alle rispettive classi di prompt e di modello. Fornisce all'Application Layer gli oggetti già configurati e pronti all'uso.

\subsubsection{Integrazione Modelli LLM (model\_strategies.py)}
Astrae e confina le complessità tecniche di rete per la comunicazione con i provider esterni (infrastruttura Zucchetti, LLM compatibili con standard OpenAI). Si occupa di iniettare le variabili d'ambiente (API keys e base URL configurati in .env e nel docker-compose.yml) e di lanciare la generazione effettiva. Grazie all'interfaccia AIModelStrategy, espone i modelli Llama 3.2 e Gemma 3 in modo polimorfico, consentendo al sistema di richiedere la generazione del testo senza doversi preoccupare dei formati di richiesta proprietari delle API.
...
Rappresenta lo strato inferiore dell'architettura, deputato alla conservazione e al recupero dei dati, dal quale dipendono indirettamente tutti i livelli superiori. In stretta coerenza con l'approccio orientato ai container (Docker) descritto nel docker-compose.yml, il backend è stato deliberatamente progettato come sistema stateless.

Non è presente alcun database server-side all'interno del backend. La persistenza dei documenti utente e dello storico delle generazioni IA è demandata interamente allo storage locale del client, sfruttando le Web Storage API.

Questa scelta assicura un drastico abbattimento dei costi infrastrutturali, una maggiore scalabilità e leggerezza dei container Docker e, soprattutto, la garanzia totale della privacy dei dati utente, poiché i testi elaborati non vengono mai memorizzati sui server dell'applicazione.
