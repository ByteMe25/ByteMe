...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        0.0.3 & 15/04/2026 & Chiara Grossele & Modifica struttura e aggiunte alla sezione Architettura backend& \\
        \hline
...
\begin{itemize}
  ...
  \item \textbf{Persistence Logic}: Conservazione dei dati.
\end{itemize}
...
\begin{itemize}
    ...
    \item Effettua i primi controlli di validità semplici e sintattici dell'input.
...
Un sottomodulo funzionale contenente un algoritmo basato su espressioni regolari (regex) atto a post-processare e "pulire" l'output fornito dai modelli linguistici. Rimuove preamboli discorsivi, garantendo formattazioni uniformi e rigorose prima che il dato risalga all'Application Layer.


\subsection{Persistence Logic}
Rappresenta lo strato inferiore dell'architettura, deputato alla conservazione e al recupero dei dati, dal quale dipendono indirettamente tutti i livelli superiori. In stretta coerenza con l'approccio orientato ai container (\gls{Docker}) descritto nel \gls{docker}-compose.yml, il backend è stato deliberatamente progettato come sistema stateless.

Non è presente alcun \gls{database} server-side all'interno del backend. La persistenza dei documenti utente e dello storico delle generazioni IA è demandata interamente allo storage locale del client, sfruttando le Web Storage \gls{API}.

Questa scelta assicura un drastico abbattimento dei costi infrastrutturali, una maggiore scalabilità e leggerezza dei container \gls{Docker} e, soprattutto, la garanzia totale della privacy dei dati utente, poiché i testi elaborati non vengono mai memorizzati sui server dell'applicazione.
