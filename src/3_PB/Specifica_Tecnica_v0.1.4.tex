...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        0.1.4 & 22/04/2026 & Giulia Barzon & Rivista sezione backend e strategy & \\
        \hline
        0.1.3 & 22/04/2026 & Giulia Barzon & Correzione e diagramma sezione Facade, rivisto MVVM & \\
        \hline
...
L'Architettura a Livelli (nota anche come \textit{Layered Architecture} o \textit{N-tier Architecture}) è uno dei pattern architetturali più consolidati e rappresenta lo standard \textit{de facto} per la progettazione di applicazioni software \textit{enterprise}.

Il principio cardine di questo pattern è la \textbf{Separation of Concerns} (Separazione delle Responsabilità): i componenti del sistema sono organizzati in strati orizzontali, dove ogni strato fornisce un'astrazione specifica e gestisce esclusivamente la logica di propria competenza. Questo approccio modulare limita il raggio d'azione dei singoli componenti, semplificando notevolmente lo sviluppo, il testing e la manutenzione dell'applicazione.
...
\subsection{Applicazione del pattern}
Il sistema \textit{Second Brain} adotta rigorosamente la \textbf{Layered Architecture} per il proprio backend. Coerentemente con i requisiti di progetto, il sistema è progettato per essere totalmente \textbf{\gls{stateless}}, delegando la persistenza dei dati interamente al client. L'architettura logica si articola nei seguenti tre livelli:
\begin{itemize}
    \item \textbf{Presentation Layer}: Espone gli \gls{endpoint} REST, riceve le richieste HTTP e restituisce le risposte al client.
    \item \textbf{Business Logic}: Core funzionale del sistema, contiene le regole di dominio e orchestra le operazioni AI in modo dinamico.
    \item \textbf{Infrastructure Layer}: Gestisce la comunicazione fisica con i provider LLM esterni e la post-elaborazione delle risposte, isolando il resto del sistema dai dettagli di rete.
\end{itemize}

% DIAGRAMMA COMPLETO DI TUTTA L'ARCHITETTURA
...
Rappresenta il punto di ingresso del backend ed è implementato nel modulo di routing di \gls{Flask}. Ha il compito di esporre gli \gls{endpoint} REST (come \texttt{/api/ai/generate}), ricevere le richieste HTTP in ingresso e restituire risposte JSON standardizzate. Questo strato non contiene logica di dominio né conoscenza della struttura dei \gls{prompt} o dei modelli LLM: si limita a validare i payload in ingresso e a delegare l'elaborazione agli strati sottostanti, intercettando e formattando eventuali eccezioni.
...
\begin{itemize}
    ...
    \item \texttt{generate\_ai\_text()}: Funzione principale del modulo, mappata sull'\gls{endpoint} POST \texttt{/api/ai/generate}. Riceve il payload JSON, estrae i parametri, interroga la mappatura delle operazioni, istanzia il modello tramite get\_model(), costruisce i \gls{prompt} e restituisce la risposta elaborata. In caso di errore, cattura l'eccezione e restituisce una risposta standardizzata con il messaggio di eccezione.
...
È il nucleo funzionale del sistema, completamente isolato dal framework web e dal protocollo HTTP. Si occupa di orchestrare il flusso dei dati e definire le regole per la costruzione dei contesti da inviare all'intelligenza artificiale.

\subsubsection{Modulo: \texttt{ai\_strategies.py} (Gestione dei \gls{prompt})}
Questo modulo contiene la logica di business per la costruzione dinamica dei \gls{prompt}. Definisce il grado di creatività (\gls{temperatura}) e le istruzioni esatte per ogni tipo di elaborazione richiesta (editoriale, traduzione, Sei Cappelli di De Bono). 
Per gestire l'enorme variabilità di queste operazioni in modo estensibile, le classi contenute in questo file (\texttt{SimplePromptStrategy}, \texttt{TranslationStrategy}, ecc.) sono state progettate applicando il \textbf{Pattern Strategy}.

\textit{Nota}: I dettagli delle singole classi sono approfonditi nella sezione relativa ai Design Pattern.


\subsubsection{Modulo: \texttt{operation\_registry.py} (Mappatura Operazioni)}
Questo modulo funge da punto di giunzione dell'intera architettura, orchestrando il legame tra le strategie di contenuto (\gls{prompt}) e le strategie di esecuzione (modelli).
...
Gestisce tutto ciò che riguarda il mondo esterno al dominio applicativo, isolando il resto del sistema dalle complessità della rete e dalla sintassi specifica delle singole API.
...
Sottomodulo funzionale contenente algoritmi di post-processing basati su espressioni regolari (regex). Il suo unico scopo è "pulire" le risposte grezze dei modelli da metatesto, tag markdown e preamboli discorsivi indesiderati prima che questi vengano passati ai livelli superiori.
...
\paragraph{Struttura e vantaggi}
...
Il pattern architetturale \textbf{Model-View-ViewModel (MVVM)} è un'evoluzione dei pattern di presentazione (come MVC e MVP) progettata per separare rigorosamente lo sviluppo dell'interfaccia utente (UI) dalla logica di business e dallo stato dell'applicazione. È un pattern di riferimento per lo sviluppo client-side moderno e si adatta perfettamente al paradigma reattivo di React.
...
Il ruolo dell'orchestratore è quello di istanziare i ViewModel, leggere lo stato dai Model e far fluire i dati e le callback verso il basso (tramite \textit{prop drilling} limitato a un solo livello) per distribuirli alle View.

\subsubsection{EditorPage: orchestratore dell'editor}
...
\begin{itemize}
    ...
    \item \textbf{Composizione degli handler}: Crea funzioni ponte tra la View e il ViewModel. Ad esempio, \texttt{handleExportWithContent} recupera prima il testo tramite \texttt{editor.getEditorText()} e lo passa al ViewModel di salvataggio; \texttt{handleCloseDocument} verifica la presenza di modifiche non salvate tramite \texttt{window.confirm} prima di resettare l'editor.
\end{itemize}

\subsubsection{HistoryPage: orchestratore dello storico}
