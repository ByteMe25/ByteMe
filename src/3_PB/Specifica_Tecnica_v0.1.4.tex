...
\begin{itemize}
    ...
    \item \texttt{generate\_ai\_text()}: Funzione principale del modulo, mappata sull'\gls{endpoint} POST \texttt{/\gls{api}/ai/generate}. Riceve il payload JSON, estrae i parametri, interroga la mappatura delle operazioni, istanzia il modello tramite get\_model(), costruisce i \gls{prompt} e restituisce la risposta elaborata. In caso di errore, cattura l'eccezione e restituisce una risposta standardizzata con il messaggio di eccezione.
...
Gestisce tutto ciò che riguarda il mondo esterno al dominio applicativo, isolando il resto del sistema dalle complessità della rete e dalla sintassi specifica delle singole \gls{API}.
...
Sottomodulo funzionale contenente algoritmi di post-processing basati su espressioni regolari (regex). Il suo unico scopo è "pulire" le risposte grezze dei modelli da metatesto, tag \gls{markdown} e preamboli discorsivi indesiderati prima che questi vengano passati ai livelli superiori.
