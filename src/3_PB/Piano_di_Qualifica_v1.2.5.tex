...
\begin{longtable}{|>{\centering\arraybackslash}p{0.10\textwidth}|>{\centering\arraybackslash}p{0.12\textwidth}|>{\centering\arraybackslash}p{0.18\textwidth}|>{\raggedright\arraybackslash}p{0.35\textwidth}|>{\raggedright\arraybackslash}p{0.15\textwidth}|}
    ...
    \hline
    \endhead
        1.2.5 & 29/04/2026 & Giulia Barzon & Aggiunti \gls{test di sistema} & \\
        \hline
        1.2.4 & 28/04/2026 & Elisa Marchioro & Aggiunti test E2E & \\
        \hline
...
Per quanto riguarda i test E2E, abbiamo scelto di usare \textbf{playwright}, che è un framework open source sviluppato da Microsoft per il testing end-to-end di applicazioni web. Permette di simulare le azioni di un utente reale nel browser, come cliccare, compilare form o navigare tra le pagine, e di verificare che tutto funzioni correttamente.

Supporta i principali browser (Chromium, Firefox e WebKit) ed esegue gli stessi test su tutti, senza bisogno di configurazioni complesse. Include anche funzionalità utili come il mocking delle chiamate di rete e la cattura automatica di screenshot o video in caso di errore.

A differenza dei test unitari, che verificano singole parti di codice, i test end-to-end controllano l’intero flusso dell’applicazione, dal frontend al backend, simulando un utilizzo reale.

\subsection{Codegen}
Playwright mette a disposizione uno strumento chiamato Codegen che permette di generare automaticamente il codice dei test registrando le azioni dell’utente nel browser. Avviando il comando npx playwright codegen <url>, si apre una finestra in cui ogni interazione (click, inserimento di testo, navigazione) viene tradotta in tempo reale in codice.
...
Gli smoke test sono test di base che servono a verificare rapidamente che le funzionalità principali dell’applicazione funzionino dopo una modifica al codice. Non coprono tutti i casi possibili, ma controllano che le operazioni essenziali non siano rotte.
...
\begin{itemize}
\item \textbf{Stampa}: questo test controlla che la funzionalità di stampa sia attivabile. Dopo il click sul pulsante “Stampa”, viene verificato che non si verifichino errori JavaScript, assicurando che l’operazione venga avviata senza problemi.
\item \textbf{Stato}: Pass.
\end{itemize}
...
\section{Test di accettazione - Fase PB}
