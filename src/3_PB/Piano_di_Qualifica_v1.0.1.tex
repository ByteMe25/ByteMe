...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        1.1.0 & 11/04/2026 & Elisa Marchioro & Aggiunti nuovi test a unit testing e miglioramento della struttura del documento & \\
        \hline
        1.0.2 & 07/04/2026 & Elisa Marchioro & Aggiunta sezione unit testing & \\
        \hline
...
L’analisi statica consiste nella verifica dei documenti e del codice senza esecuzione del software, con l’obiettivo di individuare errori sintattici, incongruenze, violazioni degli standard e problemi strutturali.

Nel progetto Second Brain, l’analisi statica viene effettuata principalmente tramite attività di inspection, supportate da checklist condivise, che guidano i verificatori nel controllo sistematico della qualità della documentazione e del codice.
...
\begin{itemize}
\item \textbf{Unit Testing}: verifica il corretto funzionamento delle singole unità software (funzioni o classi) in isolamento. L’obiettivo è individuare errori logici e garantire che ogni unità si comporti come previsto.
\item \textbf{Test di Integrazione}: verifica l’interazione tra più componenti del sistema. In questa fase si controlla che i moduli collaborino correttamente tra loro e che lo scambio di dati avvenga senza errori.
\item \textbf{Component Testing}: si concentra sulla validazione di componenti funzionali completi, considerati come unità logiche più ampie rispetto alle singole classi. Permette di verificare il comportamento di parti significative del sistema in condizioni realistiche.
\item \textbf{End-to-End Testing (E2E)}: verifica il comportamento dell’intero sistema simulando scenari d’uso reali. L’obiettivo è assicurare che il flusso completo, dalla richiesta dell’utente fino alla risposta finale, funzioni correttamente in un ambiente il più possibile simile a quello di produzione.
\end{itemize}
Questa struttura consente di individuare errori in modo progressivo, partendo dai singoli componenti fino ad arrivare alla validazione dell’intero sistema.
...
\hline
\textbf{Metrica} & \textbf{Descrizione} & \textbf{Metodo di valutazione} & \textbf{Valore atteso} \\ \hline
Tempo di risposta & Tempo necessario per generare l’output & x & x\\ \hline
...
Questo documento descrive i test relativi al modulo ai\_strategies, che definisce le strategie di costruzione dei prompt utilizzati per interagire con i modelli di intelligenza artificiale.

Il modulo fornisce diverse strategie, ciascuna progettata per uno specifico tipo di operazione (ad esempio riassunto, traduzione o analisi secondo i Sei Cappelli di De Bono). Ogni strategia è responsabile della generazione dei prompt di sistema e utente, oltre alla definizione di una temperatura adeguata al tipo di elaborazione.

I test verificano la corretta costruzione dei prompt, la coerenza delle configurazioni, la validità delle strategie registrate e il comportamento del sistema in presenza di input non validi o casi limite.
...
\prossimoUT & Il build() di BaseAIStrategy lancia NotImplementedError apposta, per obbligare le sottoclassi a reimplementarlo. Il test \gls{verifica} che questo meccanismo funzioni: se qualcuno chiama build() sulla classe base, deve ottenere quell'errore. & Pass \\ \hline
\end{tabular}

\caption{Elenco unit test con codici autogenerati}
\end{table}


\paragraph{SimplePromtStrategy}
...
\prossimoUT & \gls{Verifica} che il role appaia effettivamente nel system prompt. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che la frase fissa di comportamento, quella che dice all'AI di non fare preamboli, sia sempre presente nel system prompt, indipendentemente dal ruolo scelto. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che il task appaia nello user promt. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che il testo passato a build() finisca nello user prompt. & Pass \\ \hline
...
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & \gls{Verifica} la forma dell'output: build() deve restituire esattamente una tupla/lista di 2 elementi, entrambi stringhe. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che la lingua appaia effettivamente nello user prompt. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che il testo passato a build() finisca nello user prompt. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che il system prompt sia uguale per tutte le lingue. & Pass \\ \hline
...
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & \gls{Verifica} la forma dell'output: build() deve restituire esattamente una tupla/lista di 2 elementi, entrambi stringhe. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che il cappello selezionato appaia effettivamente nel system prompt. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che il focus appaia effettivamente nello user prompt. & Pass \\ \hline
\prossimoUT & \gls{Verifica} che il testo passato a build() finisca nello user prompt. & Pass \\ \hline
...
\prossimoUT & Tutte le strategies producano due stringhe non vuote. & Pass \\ \hline
\prossimoUT & Le funzionalità di scrittura sono istanze di SimplePromptStrategy & Pass \\ \hline
\prossimoUT & Le traduzioni sono istanze di TranslationStrategy & Pass \\ \hline
\prossimoUT & I cappelli sono istanze di DeBonoHatStrategy & Pass \\ \hline
\end{tabular}
\caption{Elenco unit test con codici autogenerati}
\end{table}
...
\subparagraph{Solo spazi e testo molto lungo}
...
\subparagraph{Caratteri speciali e unicode}
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & \gls{Verifica} che SimplePromptStrategy lanci TypeError se l'input è None. & Fail \\ \hline
\prossimoUT & \gls{Verifica} che TranslationStrategy lanci TypeError se l'input è None. & Fail \\ \hline
\prossimoUT & \gls{Verifica} che DeBonoHatStrategy lanci TypeError se l'input è None. & Fail \\ \hline
...
\subparagraph{Costruttore con valori vuoti}
...
Questo documento descrive i test relativi al modulo app, che rappresenta il backend dell’applicazione basato su Flask.

Il modulo espone un endpoint API per la generazione di contenuti tramite modelli di intelligenza artificiale. La richiesta viene elaborata selezionando l’operazione desiderata, costruendo i prompt tramite le strategie configurate e invocando il modello appropriato.

I test verificano il corretto funzionamento dell’endpoint, la validazione degli input, la gestione degli errori e la coerenza del formato delle risposte. Viene inoltre testata la funzione di pulizia dell’output, che rimuove eventuali frasi introduttive generate dal modello.
...
\paragraph{TestCleanAiResponse}
Questa sezione si concentra sulla \gls{verifica} del processo di pulizia delle risposte generate dal modello AI. In particolare, i test assicurano la rimozione di elementi superflui o prefissi tipici, al fine di restituire output chiari, coerenti e pronti per l’utilizzo lato frontend.
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & \gls{Verifica} la rimozione di prefissi introduttivi tipici dei modelli AI (es. "Ecco il riassunto:", ecc.). & Pass \\ \hline
...
\subsection{test\_model\_strategies.py}
Questo documento descrive i test relativi al modulo model\_strategies, che gestisce l’utilizzo di diversi modelli di intelligenza artificiale attraverso un’interfaccia comune.
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che la classe base non possa essere usata direttamente. & Pass \\ \hline
\prossimoUT & Verifica che get\_model restituisca sempre la stessa istanza (Singleton). & Pass \\ \hline
\prossimoUT & Verifica che la strategia Llama chiami correttamente il client OpenAI. & Pass \\ \hline
\prossimoUT & Verifica che la strategia Gemma utilizzi il modello corretto e accetti la temperatura. & Pass \\ \hline
\prossimoUT & Verifica che la strategia Llama gestisca (o sollevi) errori dall'API. & Pass \\ \hline
\prossimoUT & Verifica che get\_model distingua correttamente tra tipi diversi di modelli. & Pass \\ \hline
\prossimoUT & Verifica che le istanze create siano del tipo corretto e non si sovrappongano. & Pass \\ \hline
\prossimoUT & Verifica che venga lanciato un errore se mancano le variabili d'ambiente. & Pass \\ \hline
\prossimoUT & Verifica che la strategia gestisca una risposta vuota o mancante. & Pass \\ \hline
\prossimoUT & Verifica il corretto ripopolamento del registro interno se svuotato manualmente. & Pass \\ \hline
...
\subsection{test\_operation\_registry.py}
Questo documento descrive i test relativi al modulo operation\_registry, che definisce la configurazione delle operazioni disponibili nel sistema.

Il modulo associa a ciascuna operazione una coppia composta da:
\begin{itemize}
\item una strategia di modello AI, responsabile della generazione del testo;
\item una strategia di prompt, responsabile della costruzione delle istruzioni da fornire al modello.
\end{itemize}
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che il registro delle operazioni sia correttamente strutturato come un dizionario. & Pass \\ \hline
\prossimoUT & Accerta la presenza delle operazioni fondamentali (summary, fix\_grammar, ecc.) nel registro. & Pass \\ \hline
\prossimoUT & Verifica che l'operazione di default esista nel registro. & Pass \\ \hline
\prossimoUT & Verifica che ogni operazione abbia una classe modello valida e una strategia di prompt corretta. & Pass \\ \hline
\prossimoUT & Verifica specifica che 'summary' usi la strategia di riassunto. & Pass \\ \hline
\prossimoUT & Verifica che nessuna operazione sia configurata a None. & Pass \\ \hline
\prossimoUT & Verifica che ogni operazione usi un'istanza di strategia diversa. & Pass \\ \hline
\prossimoUT & Valida che tutte le chiavi del registro usino lo standard snake\_case (minuscole e underscore). & Pass \\ \hline
\prossimoUT & Con una simulazione di un input reale verifica che ogni strategia nel registro sia in grado di generare prompt validi. & Pass \\ \hline
\prossimoUT & Verifica che l'operazione di default abbia una config completa. & Pass \\ \hline
