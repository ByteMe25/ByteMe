...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        1.0.3 & 10/04/2026 & Elisa Marchioro & Aggiunta unit test test_app.py & \\
        \hline
...
\prossimoUT & Verifica la resilienza del sistema: se l'operazione è sconosciuta, deve eseguire il fallback sul default (summary). & Pass \\ \hline
...
\subsection{TestGenerateOperations}
...
\prossimoUT & Verifica che la richiesta vada a buon fine anche se il campo operation viene totalmente omesso dal payload. & Pass \\ \hline
...
\subsection{TestCleanAiResponse}
Questa sezione si concentra sulla verifica del processo di pulizia delle risposte generate dal modello AI. In particolare, i test assicurano la rimozione di elementi superflui o prefissi tipici, al fine di restituire output chiari, coerenti e pronti per l’utilizzo lato frontend.
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica la rimozione di prefissi introduttivi tipici dei modelli AI (es. "Ecco il riassunto:", ecc.). & Pass \\ \hline
...
\subsection{TestGenerateErrorHandling}
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che un crash del modello AI non causi un downtime del server, ma venga catturato e restituito come HTTP 500. & Pass \\ \hline
\prossimoUT & Verifica che il messaggio d'errore inviato al frontend contenga dettagli tecnici utili al debugging (testo dell'eccezione). & Pass \\ \hline
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che la risposta di successo deve sempre contenere la chiave generated\_text. & Pass \\ \hline
\prossimoUT & Verifica che anche in caso di errore (400) il frontend riceva la chiave generated\_text per mostrare il messaggio a video. & Pass \\ \hline
\prossimoUT & Verifica che l'header di risposta sia rigorosamente application/json, garantendo la compatibilità con i parser client. & Pass \\ \hline
