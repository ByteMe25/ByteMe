...
\prossimoUT & Il build() di BaseAIStrategy lancia NotImplementedError apposta, per obbligare le sottoclassi a reimplementarlo. Il test verifica che questo meccanismo funzioni: se qualcuno chiama build() sulla classe base, deve ottenere quell'errore. & Pass \\ \hline
\end{tabular}
...
\subsubsection{SimplePromtStrategy}
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica la forma dell'output: build() deve restituire esattamente una tupla/lista di 2 elementi, entrambi stringhe. & Pass \\ \hline
\prossimoUT & Verifica che il role appaia effettivamente nel system prompt. & Pass \\ \hline
\prossimoUT & Verifica che la frase fissa di comportamento, quella che dice all'AI di non fare preamboli, sia sempre presente nel system prompt, indipendentemente dal ruolo scelto. & Pass \\ \hline
\prossimoUT & Verifica che il task appaia nello user promt. & Pass \\ \hline
\prossimoUT & Verifica che il testo passato a build() finisca nello user prompt. & Pass \\ \hline
\prossimoUT & Verifica che tutte le funzionalità di scrittura hanno role e task corretti. & Pass \\ \hline
...
\subsubsection{TestTranslationStrategy}
...
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica la forma dell'output: build() deve restituire esattamente una tupla/lista di 2 elementi, entrambi stringhe. & Pass \\ \hline
\prossimoUT & Verifica che la lingua appaia effettivamente nello user prompt. & Pass \\ \hline
\prossimoUT & Verifica che il testo passato a build() finisca nello user prompt. & Pass \\ \hline
\prossimoUT & Verifica che il system prompt sia uguale per tutte le lingue. & Pass \\ \hline
\prossimoUT & Verifica che tutte le lingue supportate funzionino correttamente. & Pass \\ \hline
...
\subsubsection{TestDeBonoHatStrategy}
...
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica la forma dell'output: build() deve restituire esattamente una tupla/lista di 2 elementi, entrambi stringhe. & Pass \\ \hline
\prossimoUT & Verifica che il cappello selezionato appaia effettivamente nel system prompt. & Pass \\ \hline
\prossimoUT & Verifica che il focus appaia effettivamente nello user prompt. & Pass \\ \hline
\prossimoUT & Verifica che il testo passato a build() finisca nello user prompt. & Pass \\ \hline
\prossimoUT & Verifica che tutti i 6 cappelli funzionino correttamente. & Pass \\ \hline
...
\subsubsection{TestStrategiesDict}
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che tutte le strategies attese siano presenti & Pass \\ \hline
\prossimoUT & Verifica che tutti i valori siano istanze di BaseAIStrategy & Pass \\ \hline
...
Questa sottosezione verifica il comportamento delle strategie quando ricevono una stringa vuota in input. I test assicurano che il sistema non generi errori e continui a produrre output validi.
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.5} % Più respiro alle righe
\begin{tabular}{|l|p{12cm}|c|}
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica SimplePromptStrategy accetti la stringa vuota senza errori. & Pass \\ \hline
\prossimoUT & Verifica TranslationStrategy accetti la stringa vuota senza errori. & Pass \\ \hline
\prossimoUT & Verifica DeBonoHatStrategy accetti la stringa vuota senza errori. & Pass \\ \hline
...
\paragraph{Solo spazi e testo molto lungo}
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che SimplePromptStrategy accetti input con solo spazi. & Pass \\ \hline
\prossimoUT & Verifica che SimplePromptStrategy accetti testi molto lunghi (>10000). & Pass \\ \hline
...
Questa sottosezione verifica la capacità del sistema di gestire caratteri non standard, come emoji, simboli speciali e caratteri non latini. I test assicurano la compatibilità con input internazionali e complessi.
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.5} % Più respiro alle righe
\begin{tabular}{|l|p{12cm}|c|}
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che TranslationStrategy gestisca emoji e simboli speciali nell'input. & Pass \\ \hline
\prossimoUT & Verifica che TranslationStrategy gestisca caratteri non latini nell'input. & Pass \\ \hline
\prossimoUT & Verifica che DeBonoHatStrategy gestisca newline e tab nell'input. & Pass \\ \hline
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che SimplePromptStrategy lanci TypeError se l'input è None. & Fail \\ \hline
\prossimoUT & Verifica che TranslationStrategy lanci TypeError se l'input è None. & Fail \\ \hline
\prossimoUT & Verifica che DeBonoHatStrategy lanci TypeError se l'input è None. & Fail \\ \hline
...
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Stato} \\ \hline
\prossimoUT & Verifica che SimplePromptStrategy accetti role e task vuoti. & Pass \\ \hline
\prossimoUT & Verifica che TranslationStrategy accetti la lingua vuota. & Pass \\ \hline
\prossimoUT & Verifica che DeBonoHatStrategy accetti hat\_name e focus vuoti. & Pass \\ \hline
