...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
        ...
        \hline
        React & 19.2.0 & Libreria JavaScript per la costruzione di interfacce utente basate su componenti riutilizzabili. \\
        \hline
    \end{tabular}
...
\subsubsection{Librerie frontend}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            ...
            \textbf{Nome} & \textbf{versione} & \textbf{Descrizione} \\
            \hline
            EasyMDE & 2.20.0 & Editor Markdown con anteprima in tempo reale, basato su CodeMirror. \\
            \hline
            Axios & 1.7.9 & Client HTTP basato su Promise per effettuare richieste REST dal browser e da Node.js. \\
            \hline
            TanStack Query & - & Libreria per la gestione del fetching, caching e sincronizzazione dello stato server in React. \\
            \hline
            CSS Modules & - & Sistema di scope locale per i fogli di stile CSS, che evita conflitti tra classi in componenti diversi. \\
            \hline
            React Hot Toast & - & Libreria leggera per la visualizzazione di notifiche toast personalizzabili in React. \\
            \hline
...
\subsubsection{Linguaggio frontend}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            ...
            \hline
            TSX & - & Estensione di TypeScript che consente di scrivere markup JSX all'interno di file tipizzati, usata per i componenti React. \\
            \hline
        \end{tabular}
...
\subsubsection{Tecnologie di build}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            ...
            \textbf{Nome} & \textbf{versione} & \textbf{Descrizione} \\
            \hline
            Vite & 7.3.1 & Build tool moderno e veloce per applicazioni web, con supporto nativo a ES modules e Hot Module Replacement. \\
            \hline
        \end{tabular}
...
\subsubsection{Tecnologie di test}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            ...
            \textbf{Nome} & \textbf{versione} & \textbf{Descrizione} \\
            \hline
            Vitest & 4.1.4 & Framework di testing per applicazioni Vite, compatibile con l'API di Jest e ottimizzato per TypeScript. \\
            \hline
            Pytest & 7.0.0 & Framework di testing per Python, con sintassi semplice e supporto a fixture, parametrizzazione e plugin. \\
            \hline
            testing library React & 16.3.2 & Libreria per il testing di componenti React, che fornisce utilities per interagire con i componenti in modo simile a come un utente reale lo farebbe. \\
            \hline
        \end{tabular}
        \caption{Descrizione delle tecnologie di test}
        \label{tab:tecnologie_test}
    \end{table}
\subsubsection{Framework api backend}
    \begin{table}[H]
        \centering
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            \hline
            \textbf{Nome} & \textbf{versione} & \textbf{Descrizione} \\
            \hline
            Flask & - & Microframework Python per lo sviluppo di API REST, leggero ed estensibile tramite plugin. \\
            \hline
        \end{tabular}
        \caption{Descrizione del framework api backend}
...
\subsubsection{Linguaggio backend}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            ...
            \textbf{Nome} & \textbf{versione} & \textbf{Descrizione} \\
            \hline
            Python & 3.12-slim & Linguaggio di programmazione ad alto livello, utilizzato per la logica di business e l'implementazione delle API. \\
            \hline
        \end{tabular}
...
\subsubsection{Tecnologie deployment}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            ...
            \textbf{Nome} & \textbf{versione} & \textbf{Descrizione} \\
            \hline
            Docker & - & Piattaforma per la containerizzazione delle applicazioni, che garantisce ambienti riproducibili e portabili. \\
            \hline
        \end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Business Layer}: Costruzione del prompt, selezione del modello IA. \\
    \item \textbf{Data Layer}: Gestione delle chiamate ai modelli IA. \\
\end{itemize}

\subsection{Deployment}
Per il deployment abbiamo scelto di usare Docker, che ci permette di creare container isolati per il frontend e il backend, 
...
Per il frontend, abbiamo adottato un'architettura MVVM (Model-View-ViewModel) che si adatta bene al paradigma dei componenti di React.
