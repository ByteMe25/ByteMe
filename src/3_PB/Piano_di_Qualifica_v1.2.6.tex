...
\hline
TA-50 & \textbf{Azione:} L'utente vuole generare un grafico in base a dei dati presenti nel testo dell'editor. \newline \textbf{Esito atteso:}. Il sistema invia i dati ad un LLM che si occupa di generare un grafico del tipo indicato dall'utente e lo inserisce nell'editor; l'utente viene notificato se i dati selezionati non bastano per la creazione del grafico scelto. & ROP-F-12, ROP-F-14 & NI \\
\hline
...
\hline
TA-53 & \textbf{Azione:} L'utente vuole generare una formula matematica in base a dei dati presenti nel testo dell'editor. \newline \textbf{Esito atteso:}. Il sistema invia i dati ad un LLM che si occupa di generare una formula matematica e la inserisce nell'editor; l'utente viene notificato se si presenta un errore nella generazione. & ROP-F-17, ROP-F-18 & NI \\
\hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../graficiMetriche/PV_PB.png}
    \caption{Andamento e confronto di PV e EV nella fase PB}
\end{figure}

\textbf{PB:} Analizzando il grafico del PV/EV nella fase PB si può notare una notevole differenza di ore di lavoro tra gli \gls{sprint} della stessa fase PB e della fase precedente RTB.
In questa fase PB il gruppo ha incrementato il tempo impiegato nei vari \gls{sprint}, tenendo in considerazione che a partire dallo \gls{sprint} 9 la durata di uno \gls{sprint} è stato ridotto ad una settimana.
