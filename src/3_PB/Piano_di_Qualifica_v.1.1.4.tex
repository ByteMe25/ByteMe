...
\gls{Verifica} che HistoryPage si comporti da orchestratore corretto tra i controlli UI (input di ricerca, select, bottone elimina) e il viewmodel useHistory. L'hook viene mockato con vi.mock e riconfigurato nei singoli test tramite mockReturnValue.
...
\gls{Verifica} che EditorPage si comporti da orchestratore corretto: riceve eventi dall'UI, li delega agli hook giusti, e passa i dati nel verso corretto tra i sottocomponenti. Per farlo, mocka tutti gli hook e tutti i componenti figli con versioni minimali che espongono solo i trigger necessari ai test.
