...
\begin{itemize}
\item \textbf{Centralizzazione della configurazione}: gestisce in un unico punto la \texttt{baseURL}, il timeout e gli header standard, leggendone i valori dalle variabili d'ambiente (\texttt{VITE\_API\_BASE\_URL}, \texttt{VITE\_API\_TIMEOUT}) con fallback ai valori di default. Qualsiasi modifica ai parametri di connessione richiede interventi esclusivamente in questo file.
...
\subsubsection{Problema}
Senza un Gateway, ogni ViewModel che necessita di invocare un'operazione AI dovrebbe conoscere l'URL esatto dell'endpoint, costruire manualmente il payload JSON nel formato atteso dal backend e estrarre il testo generato dalla struttura della risposta HTTP. Questo comporterebbe una dipendenza diretta tra la logica di presentazione e i dettagli implementativi del servizio AI: qualsiasi modifica all'endpoint o al formato dei dati richiederebbe interventi su tutti i componenti che effettuano la chiamata.

\subsubsection{Applicazione}
Il Gateway astrae l'endpoint specifico (\texttt{/ai/generate}) e traduce il linguaggio del frontend in quello richiesto dal server:
\begin{itemize}
...
\item \textbf{Gateway (aiCall.ts)}: conosce la struttura del servizio AI. Costruisce il payload corretto e lo delega al Facade, specificando l'endpoint di destinazione.
