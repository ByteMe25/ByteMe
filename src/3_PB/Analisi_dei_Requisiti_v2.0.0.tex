...
\begin{longtable}{|>{\centering\arraybackslash}p{0.10\textwidth}|>{\centering\arraybackslash}p{0.12\textwidth}|>{\centering\arraybackslash}p{0.18\textwidth}|>{\raggedright\arraybackslash}p{0.35\textwidth}|>{\raggedright\arraybackslash}p{0.15\textwidth}|}
    ...
    \hline
    \endhead
        2.0.0 & 27/04/2026 & Giulia Barzon & Approvazione finale del documento & \\
        \hline
        1.5.0 & 25/04/2026 & Giulia Barzon & Modificati due requisiti & Joseph Grant \\
        \hline
        1.4.3 & 20/04/2026 & Giulia Barzon & Aggiunti RO-F-81, RO-F-82, RO-F-83, ROP-F-43, ROP-F-44, ROP-F-45 & Joseph Grant\\
        \hline
...
\begin{itemize}
  ...
  \item Capitolato d'appalto C6: \textit{Second Brain} (Zucchetti S.p.A.) (Ultimo accesso: 25/02/2026)\\ 
  \url{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C6.pdf}
  \item Regolamento del progetto didattico (Ultimo accesso: 15/03/2026)\\
        \url{https://www.math.unipd.it/~tullio/IS-1/2023/Dispense/PD2.pdf}
\end{itemize}

\subsubsection{Riferimenti informativi}
\begin{itemize}
    \item \textit{Piano di Qualifica} versione 2.0.0 (Ultimo accesso: 27/04/2026) \\
    \href{https://byteme25.github.io/ByteMe/}{PB - Piano di Qualifica v2.0.0}
    \item Verbali interni ed esterni
    \item \gls{Glossario} v2.0.0 (Ultimo accesso: 20/04/2026)\\
    ...
    \item Principi SOLID (slide prof. Cardin) (Ultimo accesso: 28/03/2026)
    \item Pattern architetturali (slide prof. Cardin) (Ultimo accesso: 28/03/2026)
...
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: L'utente tenta di annullare un'operazione quando non ci sono modifiche annullabili
    \item \textbf{Precondizioni}: L'utente non ha effettuato modifiche annullabili
...
\hline
RO-F-3 & L'editor deve compilare la sintassi \gls{Markdown} in tempo reale. & UC-1.1 \\
\hline
RO-F-4 & Deve essere presente un'area di visualizzazione del documento formattato rispetto alla sintassi \gls{Markdown}. & UC-1.1 \\
\hline
RO-F-5 & Il sistema deve permettere all'utente di selezionare porzioni di testo tramite mouse o tastiera. & UC-1.2, UC-2.6, UC-2.7, UC-2.7.*, UC-2.9, UC-2.10 \\
\hline
RO-F-6 & Il sistema deve permettere all'utente di modificare porzioni di testo selezionate. & UC-1.2 \\
\hline
RO-F-7 & Il sistema deve permettere all'utente di copiare una parte di testo selezionato. & UC-1.3 \\
\hline
RO-F-8 & Il sistema deve permettere all'utente di incollare un testo nell'area di testo. & UC-1.4 \\
\hline
RO-F-9 & Il sistema deve permettere all'utente di tagliare il testo selezionato. & UC-1.5 \\
\hline
RO-F-10 & Il sistema deve permettere all'utente di annullare l'ultima modifica effettuata. & UC-1.6 \\
\hline
RO-F-11 & Se non sono presenti operazioni annullabili, il sistema non esegue alcuna azione al comando Undo. & UC-1.6.1 \\
\hline
RO-F-12 & Il sistema deve permettere all'utente di ripristinare le modifiche precedentemente annullate. & UC-1.7 \\
\hline
RO-F-13 & Se non sono presenti operazioni ripristinabili, il sistema non esegue alcuna azione al comando Redo. & UC-1.7.1 \\
\hline
RO-F-14 & Nell'editor deve essere presente una barra degli strumenti (\gls{toolbar}) per applicare formattazioni \gls{Markdown} senza scrivere manualmente la sintassi. & UC-1.8.0, UC-1.8.* \\
\hline
RO-F-15 & Il sistema deve permettere l'applicazione del grassetto al testo tramite \gls{toolbar}. & UC-1.8.2 \\
\hline
RO-F-16 & Il sistema deve permettere l'applicazione del corsivo al testo tramite \gls{toolbar}. & UC-1.8.3 \\
\hline
RO-F-17 & Il sistema deve permettere l'applicazione della sottolineatura al testo tramite \gls{toolbar}. & UC-1.8.4 \\
\hline
RO-F-18 & Il sistema deve permettere di creare intestazioni tramite \gls{toolbar}. & UC-1.8.5 \\
\hline
RO-F-19 & Il sistema deve permettere l'inserimento di elementi di struttura (tabelle, separatori orizzontali) tramite \gls{toolbar}. & UC-1.8.6 \\
\hline
RO-F-20 & Il sistema deve permettere l'inserimento di liste numerate tramite \gls{toolbar}. & UC-1.8.7 \\
\hline
RO-F-21 & Il sistema deve permettere l'inserimento di elenchi puntati tramite \gls{toolbar}. & UC-1.8.7 \\
\hline
RO-F-22 & Il sistema deve permettere l'inserimento di link e riferimenti tramite \gls{toolbar}. & UC-1.8.8 \\
\hline
RO-F-23 & L'utente può applicare più stili contemporaneamente (es. grassetto, corsivo, sottolineatura) alla stessa sezione di testo. & UC-1.8.* \\
\hline
RO-F-24 & L'utente può annullare una formattazione di una porzione di testo selezionato riapplicandola dalla \gls{toolbar}. & UC-1.8.1 \\
\hline
RO-F-25 & L'utente può estendere la formattazione a una porzione di testo parzialmente formattato riapplicandola dalla \gls{toolbar}. & UC-1.8.0.1 \\
\hline
RO-F-26 & L'utente può creare sottoliste attivando la creazione di una lista dalla \gls{toolbar} quando il cursore è già all'interno di una lista. & UC-1.8.7.1 \\
\hline
RO-F-27 & L'utente può scegliere una combinazione di formattazioni dalla \gls{toolbar} quando non è presente testo selezionato, per applicarle al testo che scriverà successivamente. & UC-1.9.* \\
\hline
RO-F-28 & Il sistema deve permettere all'utente di selezionare porzioni di testo da inviare all'agente esterno. & UC-1.2, UC-2.* \\
\hline
RO-F-29 & Il sistema permette di inviare del testo ad un agente esterno. & UC-2.* \\
\hline
RO-F-30 & L'utente deve poter accedere alle azioni sul testo tramite agente esterno in ogni momento. & UC-2.* \\
\hline
RO-F-31 & Il testo generato viene prima mostrato come proposta, separata visivamente dal testo dell'utente. & UC-2.1 \\
\hline
RO-F-32 & Il testo in stato di proposta include le opzioni di conferma, annullamento e rigenerazione. & UC-2.1 \\
\hline
RO-F-33 & Il sistema non permette la modifica del testo generato mentre è in stato di proposta. & UC-2.* \\
\hline
RO-F-34 & Il testo generato dall'agente esterno, una volta confermato, viene inserito nella posizione del cursore se presente, altrimenti in fondo al testo. & UC-2.1.1 \\
\hline
RO-F-35 & Una proposta che viene confermata viene inserita nel documento nell'editor e la sezione di proposta viene rimossa. & UC-2.1.1 \\
\hline
RO-F-36 & Una volta generato e confermato, il testo dell'agente esterno è modificabile tramite le stesse funzionalità dell'editor disponibili per il testo utente. & UC-2.1.1 \\
\hline
RO-F-37 & Una proposta annullata comporta la rimozione del testo generato dalla sezione di proposta e la rimozione della sezione stessa. & UC-2.1.2 \\
\hline
RO-F-38 & Una proposta che viene rigenerata viene rimandata all'agente esterno con gli stessi parametri e la nuova risposta sovrascrive la proposta corrente. & UC-2.1.3 \\
\hline
RO-F-39 & Il sistema deve informare l'utente con un messaggio in caso di errore durante la comunicazione con il servizio LLM. & UC-2.1.4 \\
\hline
RO-F-40 & L'utente può annullare un'operazione AI in corso prima del suo completamento. & UC-2.1.5 \\
\hline
RO-F-41 & In seguito all'annullamento di un'operazione AI, il documento rimane nello stato precedente alla richiesta. & UC-2.1.5 \\
\hline
RO-F-42 & Se la generazione di testo si blocca prematuramente, rimane ciò che è già stato generato, in attesa che l'utente confermi, rigeneri o elimini la proposta. & UC-2.* \\
\hline
RO-F-43 & L'agente esterno applica modifiche esclusivamente al testo selezionato dall'utente. Un contesto aggiuntivo può essere fornito solo a scopo interpretativo e non deve essere alterato. & UC-2.* \\
\hline
RO-F-44 & L'utente può consultare il registro delle generazioni AI effettuate. & UC-2.0 \\
\hline
RO-F-45 & Il sistema deve gestire il caso in cui lo storico delle generazioni AI sia vuoto, informando l'utente con un messaggio appropriato. & UC-2.0.1 \\
\hline
RO-F-46 & Ogni generazione di testo dell'agente esterno viene inclusa nel registro delle generazioni, incluse quelle annullate dall'utente. & UC-2.0, UC-2.1 \\
\hline
RO-F-47 & Il sistema deve permettere all'utente di visualizzare il testo completo di una generazione dallo storico AI. & UC-2.0.3 \\
\hline
RO-F-48 & L'utente deve poter riassumere tutto o una parte selezionata del testo usando un agente esterno. & UC-2.2 \\
\hline
RO-F-49 & L'utente deve poter usare un agente esterno per riscrivere tutto o una parte selezionata del testo migliorandolo seguendo determinate specifiche come sintassi, stile e chiarezza. & UC-2.3 \\
\hline
RO-F-50 & L'utente deve poter tradurre il testo usando un agente esterno in: inglese, italiano, tedesco, francese, spagnolo e cinese mandarino. & UC-2.4.* \\
\hline
RO-F-51 & Se l'utente avvia una traduzione senza selezionare la lingua di destinazione, il sistema lo notifica e blocca l'operazione. & UC-2.4.1 \\
\hline
RO-F-52 & Il sistema permette di avviare la correzione grammaticale del testo tramite agente esterno. & UC-2.5 \\
\hline
RO-F-53 & Se non vengono rilevati errori grammaticali, il sistema informa l'utente dell'esito positivo della correzione. & UC-2.5.1 \\
\hline
RO-F-54 & Il sistema consente all'utente di avviare l'analisi critica del testo selezionato o dell'intero documento utilizzando il cappello di De Bono scelto. & UC-2.6 \\
\hline
RO-F-55 & Se il testo è insufficiente per l'analisi critica, il sistema notifica l'utente e interrompe l'operazione. & UC-2.6.1 \\
\hline
RO-F-56 & Se l'utente avvia l'analisi critica senza selezionare un cappello di De Bono, il sistema lo notifica e blocca l'operazione. & UC-2.6.2 \\
\hline
RO-F-57 & Il sistema permette l'invio di un \gls{prompt} scritto dall'utente per la generazione di testo tramite agente esterno. & UC-2.7 \\
\hline
RO-F-58 & Il sistema permette la modifica del \gls{prompt} da inviare all'agente esterno prima dell'invio. & UC-2.7 \\
\hline
RO-F-59 & L'agente esterno può generare testo basandosi su un \gls{prompt} fornitogli dall'utente. & UC-2.7.* \\
\hline
RO-F-60 & Se il \gls{prompt} fornito dall'utente è vuoto o incompleto, il sistema notifica l'utente e impedisce l'invio. & UC-2.7.1 \\
\hline
RO-F-61 & L'utente può caricare un file dal proprio dispositivo all'interno dell'editor. & UC-3.* \\
\hline
RO-F-62 & Il sistema deve supportare il caricamento di file in formato \gls{Markdown} (\texttt{.md}, \texttt{.\gls{markdown}}) e testo semplice (\texttt{.txt}). & UC-3.0, UC-3.0.1, UC-3.0.2 \\
\hline
RO-F-63 & Il sistema deve supportare il caricamento tramite dialogo di esplorazione file (file picker). & UC-3.0.4 \\
\hline
RO-F-64 & Prima di procedere con il caricamento di un file, il sistema chiede conferma all'utente. & UC-3.0 \\
\hline
RO-F-65 & Il sistema deve informare l'utente con un messaggio di esito (successo o errore) al termine di ogni tentativo di caricamento file. & UC-3.1, UC-3.2 \\
\hline
RO-F-66 & Il sistema deve validare il formato del file selezionato e impedire il caricamento di formati non supportati. & UC-3.1 \\
\hline
RO-F-67 & Il sistema deve validare le dimensioni del file selezionato e impedire il caricamento di file che superano la dimensione massima consentita. & UC-3.2 \\
\hline
RO-F-68 & Il sistema deve impedire il caricamento di file corrotti o illeggibili. & UC-3.1 \\
\hline
RO-F-69 & Il sistema deve permettere all'utente di rimuovere il file che ha caricato. & UC-3.3 \\
\hline
RO-F-70 & L'utente può annullare l'operazione di rimozione del file caricato, lasciando l'editor nel suo stato precedente. & UC-3.3.1 \\
\hline
RO-F-71 & Il sistema deve essere in grado di convertire il documento da \gls{Markdown} al formato selezionato dall'utente. & UC-4.0 \\
\hline
RO-F-72 & Il sistema deve permettere di scaricare il file nel dispositivo dell'utente. & UC-4.0 \\
\hline
RO-F-73 & L'operazione di esportazione o stampa non deve alterare né chiudere il documento originale nell'editor. & UC-4.* \\
\hline
RO-F-74 & Il sistema deve informare l'utente con un messaggio di esito al termine di ogni operazione di esportazione. & UC-4.0 \\
\hline
RO-F-75 & Il sistema deve consentire l'esportazione del documento in formato \gls{Markdown} (\texttt{.md}). & UC-4.0.2 \\
\hline
RO-F-76 & Il sistema deve controllare che il range di pagine inserito dall'utente sia valido (non superiore al numero di pagine del documento e non negativo). & UC-4.0.6, UC-4.0.7 \\
\hline
RO-F-77 & Il sistema deve permettere di collegare e inviare il documento a un dispositivo di stampa. & UC-4.1 \\
\hline
RO-F-78 & Il sistema deve informare l'utente con un messaggio in caso di errore durante l'invio del documento alla stampante. & UC-4.1 \\
\hline
RO-F-79 & Il sistema deve garantire la persistenza locale del testo nell'editor tramite browser storage (LocalStorage). & UC-1.* \\
\hline
RO-F-80 & Il sistema deve garantire la persistenza locale dello storico delle generazioni AI tramite browser storage (\gls{Zustand} persist). & UC-2.* \\
\hline
RO-F-81 & Il sistema deve impedire il caricamento di file di dimensioni superiori a 5MB. & UC-3.2 \\
\hline
...
\hline
RD-F-16 & Il sistema deve informare l'utente con un messaggio se l'editor non risponde ai comandi. & UC-1.0.2 \\
\hline
RD-F-17 & Il sistema deve informare l'utente con un messaggio se il pannello di anteprima non si aggiorna correttamente. & UC-1.1.1 \\
\hline
...
\hline
ROP-F-43 & Il sistema deve permettere all'utente di modificare il nome del documento corrente direttamente dall'interfaccia di lavoro. & UC-1.0 \\
\hline
ROP-F-44 & Il sistema deve permettere all'utente di cercare testo all'interno del registro delle generazioni AI tramite una barra di ricerca. & UC-2.0 \\
\hline
ROP-F-45 & Il sistema deve permettere all'utente di filtrare le voci del registro delle generazioni AI in base al tipo di operazione effettuata. & UC-2.0 \\
\hline
\addlinespace[1em]
...
\hline
\endhead
UC-1.0 & Scrittura nell'editor & RO-F-1, RO-F-81, ROP-F-43 \\
\hline
UC-1.0.1 & Gestione perdita connessione Internet & RO-F-2 \\
\hline
UC-1.0.2 & Gestione errore di risposta dell'editor & RD-F-16 \\
\hline
UC-1.1 & Visualizzazione anteprima in tempo reale & RO-F-4, RO-F-5 \\
\hline
UC-1.1.1 & Gestione errore nell'anteprima & RD-F-17 \\
\hline
...
\hline
UC-2.0 & Visualizzazione storico generazioni AI & RO-F-46, RO-F-48, RO-F-82, ROP-F-44, ROP-F-45 \\
\hline
...
\hline
UC-3.2 & Gestione file troppo grandi & RO-F-67, RO-F-69, RO-F-83, RD-F-6 \\
\hline
...
\hline
Funzionali & 81 & 17 & 45 & 143 \\
\hline
Vincolo & 4 & 0 & 0 & 4 \\
\hline
Qualità & 10 & 0 & 0 & 10 \\
\hline
\textbf{Totale} & 95 & 17 & 45 & 157 \\
\hline
\end{tabular}
