...
\subsubsection{Problema}
Senza questo modulo, ogni ViewModel che necessita di invocare un'operazione AI dovrebbe conoscere l'URL esatto dell'endpoint, costruire manualmente il payload JSON nel formato atteso dal backend ed estrarre il testo generato dalla struttura della risposta HTTP. Questo comporterebbe una dipendenza diretta tra la logica di presentazione e i dettagli implementativi del servizio AI: qualsiasi modifica all'endpoint o al formato dei dati richiederebbe interventi su tutti i componenti che effettuano la chiamata.

\subsubsection{Applicazione}
Il modulo astrae l'endpoint specifico (/ai/generate) e traduce il linguaggio del frontend in quello richiesto dal server:
\begin{itemize}
