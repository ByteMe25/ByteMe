...
\begin{itemize}
    ...
    \item \texttt{build(text: str, prompt: str)}: Metodo astratto che impone a ogni sottoclasse di restituire una tupla contenente due stringhe: il \gls{System prompt} (il ruolo dell'IA) e lo \gls{User prompt} (il task specifico).
\end{itemize}
\end{itemize}
...
\end{itemize}
    \item\textbf{Metodi:}
\begin{itemize}
    \item \texttt{build(text: str, prompt: str)}: Combina \texttt{role} e \texttt{task} con il testo in ingresso per formare la tupla richiesta.
\end{itemize}
\end{itemize}
...
\item \textbf{Metodi:}
\begin{itemize}
    \item build(text: str, prompt: str): Utilizza le f-string per iniettare i valori di hat\_name e focus all'interno dei \gls{prompt}.
\end{itemize}
\end{itemize}

\paragraph{DistantWritingStrategy}
Sottoclasse concreta dedicata alla funzionalità di generazione creativa di contenuti. A differenza delle altre strategie, gestisce la particolarità di ricevere un input testuale diretto dall'utente (prompt) accompagnato da un testo di contesto opzionale. La temperatura viene sovrascritta a 0.7 per favorire una maggiore creatività del modello.
\begin{itemize}
    \item \textbf{Attributi:}
    \begin{itemize}
    ...
    \item \textbf{Metodi:}
    \begin{itemize}
        \item build(text: str, prompt: str): Implementa la logica di costruzione unendo l'istruzione dell'utente (prompt) e l'eventuale text fornito come contesto. Imposta inoltre un System Prompt rigoroso per evitare frasi introduttive.
    \end{itemize}
\end{itemize}
