...
\begin{longtable}{|>{\centering\arraybackslash}p{0.10\textwidth}|>{\centering\arraybackslash}p{0.12\textwidth}|>{\centering\arraybackslash}p{0.18\textwidth}|>{\raggedright\arraybackslash}p{0.35\textwidth}|>{\raggedright\arraybackslash}p{0.15\textwidth}|}
    ...
    \hline
    \endhead
        0.2.8 & 09/05/2026 & Elisa Marchioro & Modifiche sezione \gls{API} & \\
        \hline
...
Senza un punto di accesso centralizzato, ogni componente o ViewModel dovrebbe istanziare e orchestrare autonomamente tutte queste entità. Questo approccio porterebbe a duplicazione del codice, comportamenti eterogenei in caso di errore e un forte accoppiamento tra la logica di presentazione e i dettagli implementativi del trasporto HTTP.
...
\paragraph{Collaborazione tra i componenti}
...
\item \textbf{aiCall.ts}: conosce la struttura del servizio AI. Costruisce il payload corretto e lo delega al Facade, specificando l'\gls{endpoint} di destinazione.
...
La progettazione del frontend di \textit{Second Brain} ha fatto uso di soluzioni 
architetturali consolidate orientate alla separazione delle responsabilità, 
al disaccoppiamento tra i moduli e alla gestione ordinata della complessità 
dell'interfaccia utente. L'obiettivo è stato mantenere View e ViewModel il più 
possibile agnostici rispetto ai dettagli implementativi, favorendo codice 
modulare, leggibile e manutenibile.
