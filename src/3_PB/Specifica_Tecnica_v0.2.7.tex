...
\begin{itemize}
    ...
    \item \texttt{generate\_ai\_text()}: Funzione principale del modulo, mappata sull'\gls{endpoint} POST \texttt{/\gls{api}/ai/generate}. Riceve il payload JSON, estrae i parametri e agisce da \textit{orchestratore}: interroga la mappatura delle operazioni, recupera l'istanza del modello e delega alla Business Logic la costruzione dei \gls{prompt}. Infine, restituisce la risposta elaborata e ripulita al client. In caso di errore, cattura l'eccezione e restituisce una risposta HTTP standardizzata con il messaggio di errore.
...
Questo modulo funge da punto di giunzione dell'intera architettura, associando le strategie di prompt alle strategie di esecuzione (modelli LLM) nel rispetto delle regole di dominio.
