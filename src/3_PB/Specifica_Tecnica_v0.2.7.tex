...
\begin{itemize}
    ...
    \item \textbf{Infrastructure Layer}: strato inferiore dell'architettura, che incapsula e nasconde agli strati superiori tutti i dettagli tecnologici di accesso ai servizi esterni. In assenza di un \gls{database} relazionale, questo strato svolge il ruolo analogo a quello di un Persistence Layer classico: gestisce le credenziali, stabilisce le connessioni con i provider LLM tramite protocollo OpenAI-compatibile e traduce le risposte nel formato atteso dallo strato superiore.
    \textit{Secondo la documentazione ufficiale Microsoft sull'architettura delle applicazioni web, le chiamate di rete a servizi esterni rientrano esplicitamente nelle responsabilità dell'Infrastructure Layer~\cite{microsoft_web_arch}. Sostituire un provider LLM richiede modifiche esclusivamente a questo strato.}
\end{itemize}
...
Il pattern \textbf{Facade} è un design pattern strutturale il cui scopo è fornire un'interfaccia unica e semplice per un sottosistema complesso. L'obiettivo non è nascondere o bloccare l'accesso alle funzionalità di basso livello, ma fornire una "vista di default" semplificata che riduca il numero di classi con cui il client deve interagire. Nel nostro sistema, l'adozione di questo pattern comporta due vantaggi architetturali primari: realizza un accoppiamento lasco (\textit{loose coupling}) tra i ViewModel e l'infrastruttura di rete, e diminuisce la complessità generale favorendo una rigida stratificazione (layered architecture).

\subsubsection{Problema}
Il sottosistema di networking della nostra applicazione è composto da diverse entità che devono collaborare: il client HTTP (Axios), i meccanismi di intercettazione delle richieste, il formattatore degli errori e i token di cancellazione (AbortSignal). 
...
Il file \texttt{apiClient.ts} funge da Facade per il sottosistema di networking. Il suo scopo è orchestrare le varie componenti infrastrutturali fornendo un'interfaccia semplificata ai livelli superiori. Invece di dover gestire le singole entità di rete, i ViewModel si interfacciano unicamente con il Facade, il quale si occupa di:
\begin{itemize}
...
\subsubsection{Applicazione}
Il modulo astrae l'\gls{endpoint} specifico (/ai/generate) e traduce il linguaggio del frontend in quello richiesto dal server:
\begin{itemize}
