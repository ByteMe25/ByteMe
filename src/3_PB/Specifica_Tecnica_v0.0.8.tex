...
\subsection{Librerie frontend}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|>{\raggedright\arraybackslash}p{0.35\textwidth}|p{0.18\textwidth}|}
            ...
            \hline
            Lucide-React & 1.8.0 & Libreria di icone vettoriali utilizzata per la UI dell'applicazione.\\
            \hline
        \end{tabular}
...
Costituisce il punto di ingresso dell'applicazione lato server. Nel progetto, questo livello è implementato nel modulo di routing del framework Flask (\texttt{app.py}).
...
\begin{itemize}
    \item \textbf{Gestione dei Prompt}: Definizione delle istruzioni per l'IA;
...
Implementa il pattern \textbf{Registry}. Contiene le strutture dati (\texttt{OperationConfig}) che associano dinamicamente gli identificatori delle operazioni alle corrette strategie di prompt e ai modelli corrispondenti. Questo componente funge da "collante" tra i livelli, fornendo all'Application Layer oggetti preconfigurati e pronti all'uso.

\subsubsection{Integrazione degli LLM (model\_strategies.py)}
Astrae le complessità di rete per la comunicazione con i provider esterni (infrastruttura Zucchetti o API compatibili OpenAI). Gestisce l'iniezione delle variabili d'ambiente (\texttt{API key} e \texttt{base URL}) e l'invocazione dei modelli. Grazie all'interfaccia \texttt{AIModelStrategy}, espone modelli come \textbf{Llama 3.2} e \textbf{Gemma 3} in modo polimorfico.
...
\section{Deployment}
Il sistema \textit{Second Brain} utilizza la tecnologia di containerizzazione \textbf{Docker} per garantire l'isolamento dei componenti, la portabilità tra diversi ambienti di sviluppo e produzione e la gestione semplificata delle dipendenze.

\subsection{Strategia di Containerizzazione}
L'architettura è suddivisa in due micro-servizi orchestrati tramite \texttt{docker-compose}, che permette di definire reti virtuali interne e volumi per la persistenza o il \textit{hot-reloading} durante lo sviluppo.
...
\begin{itemize}
    \item \textbf{Stage 1 (Build)}: Utilizza \texttt{node:20-alpine} per installare le dipendenze ed eseguire la build di produzione tramite Vite. L'uso di Alpine Linux garantisce velocità e leggerezza nel processo di CI/CD.
    \item \textbf{Stage 2 (Production)}: Il risultato della build viene copiato in un'immagine \texttt{nginx:alpine}. Nginx funge da web server statico ad alte prestazioni, configurato appositamente per gestire il routing \textit{client-side} tipico delle Single Page Application (SPA).
\end{itemize}

\subsection{Orchestrazione: Docker Compose}
Il file \texttt{docker-compose.yml} coordina l'avvio sincronizzato dei servizi e la gestione delle risorse:
\begin{itemize}
    \item \textbf{Mapping delle porte}: Il frontend è reso accessibile all'host sulla porta \texttt{8080}, mappata sulla porta interna \texttt{80} di Nginx.
    ...
    \item \textbf{Sviluppo Agile}: Durante la fase di sviluppo, viene utilizzato un \textit{bind mount} per permettere al server Flask di riflettere istantaneamente le modifiche al codice senza dover ricostruire l'immagine.
\end{itemize}

\newpage

\section{Architettura del frontend}
Il frontend del sistema è stato sviluppato utilizzando \textbf{React 19}, adottando il pattern architetturale \textbf{MVVM} (Model-View-ViewModel). Questa scelta permette di separare nettamente la logica di business dalla logica di presentazione, facilitando il testing dei componenti e la manutenibilità del codice.

L'organizzazione del progetto segue i principi della \textit{Feature-based Architecture} (ispirata a \textbf{Bulletproof React}), in cui ogni funzionalità del sistema (come l'Editor o lo Storico) è isolata in moduli autocontenuti che includono i propri componenti, hook e store.
...
Per garantire la persistenza dei dati senza un database backend, il Model integra il middleware \texttt{persist}, che sincronizza automaticamente lo stato dello store con il \textbf{LocalStorage} del browser, assicurando che i documenti e lo storico siano disponibili anche dopo il riavvio della sessione.

\subsection{View}
La View è costituita da componenti React dichiarativi e per la maggior parte "dumb" (o \textit{presentational}). Il loro unico compito è il rendering dell'interfaccia utente basandosi sulle \textit{props} ricevute. Le View non contengono logica di business né effettuano chiamate dirette alle API; esse delegano ogni interazione dell'utente al ViewModel sottostante tramite callback, garantendo che lo strato di presentazione rimanga snello e facilmente testabile.
...
\begin{itemize}
    ...
    \item Gestire la logica di interazione complessa (ad esempio, l'orchestrazione delle API AI tramite la Facade \texttt{aiCall}).
    \item Manipolare direttamente le istanze di librerie esterne, come \textbf{EasyMDE} per l'editor Markdown, isolandone la complessità tecnica all'interno di hook dedicati.
\end{itemize}
...
\begin{itemize}
    ...
    \item Gestire manualmente la logica di accoppiamento tra prompt e modelli.
...
Il modulo funge da punto di giunzione dell'intera architettura, orchestrando il legame tra le strategie di contenuto (prompt) e le strategie di esecuzione (modelli).
...
\begin{itemize}
    ...
    \item \texttt{prompt\_strategy} (\texttt{BaseAIStrategy}): è un'istanza preconfigurata della strategia di \gls{prompt} specifica per quell'operazione.
\end{itemize}
