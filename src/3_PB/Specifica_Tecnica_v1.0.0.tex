...
\begin{longtable}{|>{\centering\arraybackslash}p{0.10\textwidth}|>{\centering\arraybackslash}p{0.12\textwidth}|>{\centering\arraybackslash}p{0.18\textwidth}|>{\raggedright\arraybackslash}p{0.35\textwidth}|>{\raggedright\arraybackslash}p{0.15\textwidth}|}
    ...
    \hline
    \endhead
        1.0.0 & 11/05/2026 & Giulia Barzon & Approvazione finale & \\
        \hline
        0.2.8 & 09/05/2026 & Elisa Marchioro & Modifiche sezione \gls{API} & Giuia Barzon, Tommaso Tombacco\\
        \hline
        0.2.7 & 08/05/2026 & Giulia Barzon & Piccolo fix sezione layered e deployment & Elisa Marchioro, Joseph Grant\\
        \hline
        ...
        \hline
        0.1.6 & 23/04/2026 & Elisa Marchioro & Sistemata formattazione, aggiunta sezione \gls{API} e aggiunta diagrammi backend & Chiara Grossele \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|p{0.10\textwidth}|p{0.55\textwidth}|}
        ...
        \hline
        Axios & 1.7.9 & Client HTTP basato su Promise per effettuare richieste HTTP dal browser. \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|p{0.10\textwidth}|p{0.55\textwidth}|}
        ...
        \textbf{Nome} & \textbf{Versione} & \textbf{Descrizione} \\
        \hline
        \gls{Flask} & 3.1.3 & Microframework Python per lo sviluppo di \gls{API}, leggero ed estensibile tramite plugin. \\
        \hline
    \end{tabular}
...
Il sistema \textit{Second Brain} si basa su un'architettura di tipo \textbf{Client-Server}, distribuita fisicamente su nodi (container) logici distinti, che comunicano tramite protocollo HTTP.

\begin{itemize}
    \item \textbf{Client (Frontend)}: applicazione \gls{React} eseguita interamente nel browser dell'utente. Si occupa esclusivamente della presentazione e dell'interazione con l'utente: raccoglie gli input, invia le richieste al server e aggiorna l'interfaccia con i risultati ricevuti. Non contiene logica di dominio né accede direttamente ai modelli LLM.
    
    \item \textbf{Server (Backend)}: applicazione \gls{Flask} eseguita lato server. Espone un'interfaccia HTTP, riceve le richieste del client, orchestra la logica applicativa e di dominio (selezione della strategia, costruzione dei \gls{prompt}, invocazione dei modelli LLM) e restituisce i risultati al client in formato JSON.
\end{itemize}

Questa separazione garantisce la \textbf{\gls{Separation of Concerns}}: il client gestisce esclusivamente la presentazione, mentre il server centralizza la logica, l'orchestrazione e l'integrazione con i servizi esterni (provider LLM). Grazie alla natura \textbf{\gls{stateless}} delle comunicazioni, ogni richiesta HTTP contiene tutte le informazioni necessarie per essere elaborata indipendentemente, rendendo il backend replicabile e distribuibile senza necessità di gestione condivisa dello stato di sessione.

\newpage

\section{Deployment}
Il pattern Client-Server descritto nella sezione precedente è realizzato fisicamente tramite la tecnologia di containerizzazione \textbf{\gls{Docker}}, che garantisce l'isolamento dei componenti, la portabilità tra ambienti di sviluppo e produzione e la gestione semplificata delle dipendenze.

Ciascuno dei due nodi logici (client e server) corrisponde a un container 
\gls{Docker} distinto, orchestrati tramite \texttt{\gls{docker}-compose}. 
Entrambi i nodi adottano un'\textbf{architettura monolitica}: il backend 
è un singolo processo \gls{Flask} che incapsula l'intera logica applicativa, 
mentre il frontend è una Single Page Application compilata in un unico bundle 
statico servito da \gls{Nginx}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../UMLimg/imgDocker.jpg}
...
\section{Backend: architettura}
\subsection{Layered Architecture}
Il backend del sistema \textit{Second Brain} è stato progettato adottando il pattern architetturale \textbf{Layered Architecture} (o Architettura a Livelli).
...
\begin{itemize}
    \item \textbf{Application Logic}: punto di ingresso del backend. Riceve le richieste HTTP in arrivo dal client, valida i payload, gestisce il routing e orchestra il flusso verso gli strati sottostanti. Non contiene logica di dominio né conosce i dettagli implementativi delle strategie o dei provider LLM.
    \item \textbf{Business Logic}: nucleo funzionale del sistema, completamente isolato dal framework web e da qualsiasi dettaglio di I/O. Implementa le regole di dominio (costruzione dei \gls{prompt} per le operazioni AI) utilizzando esclusivamente astrazioni dei servizi esterni, senza conoscerne i dettagli tecnologici.
    \item \textbf{Infrastructure Layer}: strato inferiore dell'architettura, che incapsula e nasconde agli strati superiori tutti i dettagli tecnologici di accesso ai servizi esterni. Questo strato gestisce le credenziali, stabilisce le connessioni con i provider LLM tramite protocollo OpenAI-compatibile e traduce le risposte nel formato atteso dallo strato superiore.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Diagramma UML dell'architettura del backend: layered architecture}
...
\begin{itemize}
    \item \texttt{app}: Istanza dell'applicazione \gls{Flask}, configurata con CORS abilitato per permettere la comunicazione sicura col nodo client (frontend).
    
    \item \texttt{generate\_ai\_text()}: Funzione principale del modulo, mappata sull'\gls{endpoint} POST \texttt{/\gls{api}/ai/generate}. Riceve il payload JSON, estrae i parametri e agisce da \textit{\gls{orchestratore}}: interroga la mappatura delle operazioni, recupera l'istanza del modello e delega alla Business Logic la costruzione dei \gls{prompt}. Infine, restituisce la risposta elaborata e ripulita al client. In caso di errore, cattura l'eccezione e restituisce una risposta HTTP standardizzata con il messaggio di errore.
    
    \item \texttt{root()}: Funzione mappata sull'\gls{endpoint} GET \texttt{/}, utilizzata come \textit{health check} per verificare che il server sia attivo. Restituisce lo stato del servizio e la lista delle operazioni supportate dal Mapper.
\end{itemize}
...
La \textbf{Business Logic} rappresenta il nucleo funzionale del sistema, completamente isolato dal framework web e dal protocollo HTTP. Questo strato si occupa di incapsulare le politiche applicative e definire le regole di dominio per la costruzione dei contesti da inviare agli LLM. L'architettura di questo livello è suddivisa in moduli specializzati che utilizzano Design Pattern per gestire l'elevata variabilità delle operazioni richieste in modo flessibile.
...
Questo modulo funge da punto di giunzione dell'intera architettura, associando le strategie di \gls{prompt} alle strategie di esecuzione (modelli LLM) nel rispetto delle regole di dominio.
...
Questa struttura avrebbe violato il principio \textbf{Open-Closed (OCP)} e il principio di \textbf{Inversione delle Dipendenze (DI)}, rendendo l'aggiunta di una nuova operazione un processo rischioso e richiedendo la modifica del codice di routing principale ogni volta che viene introdotta una nuova funzionalità. Per ovviare a questo, si è optato per una mappatura centralizzata.
...
Sottomodulo funzionale che agisce come \textbf{Domain Service} di supporto. Utilizza espressioni regolari (regex) per validare e "ripulire" l'output testuale grezzo generato dagli LLM. Il suo scopo è garantire che il testo restituito al client sia privo di metatesto, preamboli discorsivi o tag \gls{markdown} residui, assicurando la coerenza del dominio.

% ===================
% INFRASTRUCTURE LAYER
% ===================
\subsubsection{Infrastructure Layer}
L'\textbf{Infrastructure Layer} rappresenta il confine inferiore 
dell'architettura. È completamente isolato dalla Business Logic tramite 
l'astrazione \texttt{AIModelStrategy}: gli strati superiori conoscono 
solo l'interfaccia, mai i dettagli implementativi del provider sottostante.
Questo strato si occupa di gestire tutti i dettagli tecnologici di 
accesso ai servizi LLM esterni: iniezione delle chiavi \gls{API} 
dall'ambiente, costruzione del client HTTP, impacchettamento della 
richiesta nel formato OpenAI-compatibile e restituzione della risposta 
grezza allo strato superiore.
...
La progettazione del backend di \textit{Second Brain} si basa sull'applicazione di design pattern comportamentali. L'obiettivo primario di questa scelta architetturale è garantire l'estensibilità del sistema: l'aggiunta di nuove operazioni AI o l'integrazione di nuovi modelli LLM deve poter avvenire senza alterare il codice di routing esistente, nel pieno rispetto dei principi SOLID (in particolare l'\textit{Open-Closed Principle} e il \textit{Single Responsibility Principle}).
...
\paragraph{Vantaggi}
L'adozione di questo pattern porta due importanti vantaggi:
\begin{enumerate}
...
\paragraph{Classi concrete (ZucchettiDeepSeekStrategy, Gemma3Strategy, Qwen3Strategy)}
...
\section{Frontend: gestione degli stati del widget AI}
...
\subsection{Soluzione: rendering delegato e stati polimorfi}
...
\begin{figure}[H]
    ...
    \caption{Diagramma UML che mostra la gestione dei Widget AI}
...
\subsection{Orchestrazione: store globale e ViewModel}
...
\subsubsection{Lo store (\texttt{aiWidgetStore})}
...
\subsubsection{ViewModel (\texttt{useAiOperations})}
...
\section{Interfaccia \gls{API}}
Il sistema adotta un'interfaccia \gls{API} basata sullo stile architetturale
\textbf{\gls{RPC}} (Remote Procedure Call), specificatamente nella variante
\textbf{JSON-RPC-like} su HTTP.

\subsection{Confronto REST e RPC}
REST e RPC sono due stili architetturali distinti per la progettazione
delle \gls{API}. Entrambi utilizzano HTTP come protocollo sottostante
e JSON come formato di scambio, ma differiscono nel modello mentale con cui organizzano la comunicazione.

\paragraph{REST: Representational State Transfer}
REST è uno stile architetturale formalizzato da Roy Fielding nel 2000. Il suo modello mentale è orientato alle \textbf{risorse} (sostantivi): ogni entità del dominio è identificata da un URI univoco e le operazioni su di essa sono espresse dai verbi HTTP standard
(\texttt{GET}, \texttt{POST}, \texttt{PUT}/\texttt{PATCH}, \texttt{DELETE}).
\begin{itemize}
    \item \textbf{Domanda chiave}: «Qual è l'oggetto coinvolto?»
    \item \textbf{Esempio}: \texttt{POST /\gls{api}/articles} crea un articolo; \texttt{DELETE /\gls{api}/articles/42} lo elimina.
    \item \textbf{Adatto a}: operazioni CRUD su risorse persistenti, \gls{API} pubbliche, sistemi che beneficiano della cache HTTP nativa.
\end{itemize}

\paragraph{RPC: Remote Procedure Call}
RPC è un paradigma in cui il client invoca direttamente una \textbf{funzione} (procedura) sul server, come se fosse una chiamata locale. Il modello mentale è orientato alle \textbf{azioni} (verbi): l'operazione da eseguire è specificata nel corpo della richiesta, non nell'URL.
\begin{itemize}
    \item \textbf{Domanda chiave}: «Quale funzione devo chiamare?»
    \item \textbf{Esempio}: \texttt{POST /\gls{api}/generate} con body
    \texttt{\{"operationId": "summary", "text": ""\}}.
    \item \textbf{Adatto a}: azioni su dati, chiamate interne, domini orientati alle procedure come l'elaborazione AI.
\end{itemize}

\subsection{Motivazione della scelta: JSON-RPC-like}
Le operazioni AI del sistema (\textit{summary}, \textit{rewrite}, \textit{translate}, analisi con i Sei Cappelli di De Bono...) sono \textbf{azioni su un testo}, non operazioni CRUD su risorse persistenti. Non esiste un «documento di riassunto» da creare, aggiornare o eliminare: esiste una trasformazione da applicare. Questo dominio è intrinsecamente orientato alle procedure, rendendo RPC il pattern semanticamente più corretto.

Tutta la comunicazione AI transita attraverso un unico \gls{endpoint}:
\begin{center}
    \texttt{POST /\gls{api}/ai/generate}
\end{center}
Il tipo di operazione da eseguire sul server è specificato nel corpo della richiesta (body JSON) tramite il campo \texttt{operationId}:
\begin{verbatim}
{
  "operationId": "summary",
...
A titolo comparativo, un'implementazione REST avrebbe richiesto \gls{endpoint} separati per ogni operazione:
\begin{verbatim}
    POST /api/ai/summary
    POST /api/ai/rewrite
    POST /api/ai/fix-grammar
    POST /api/ai/translate/it
\end{verbatim}
Questa struttura non avrebbe apportato vantaggi concreti (nessuna risorsa identificabile, nessuna cacheabilità) a fronte di una maggiore complessità di routing lato backend e frontend.

L'interfaccia rispetta comunque i seguenti principi fondamentali del protocollo HTTP:
\begin{itemize}
    \item \textbf{Statelessness}: ogni richiesta è autocontenuta; il server non mantiene stato tra richieste successive.
    \item \textbf{Uso corretto dei metodi HTTP}: \texttt{POST} per operazioni con effetti sul server; \texttt{GET} per l'health check.
    \item \textbf{Formato JSON}: scambio dati standardizzato e interoperabile tra sistemi eterogenei.
    \item \textbf{Separazione delle responsabilità}: il client gestisce la presentazione; il server gestisce logica, orchestrazione e integrazione con i provider LLM.
    \item \textbf{Scalabilità}: la natura \gls{stateless} permette di replicare il backend senza gestione di sessioni condivise.
\end{itemize}

\subsection{Comunicazione client–server (Axios e Servizi)}
Nel frontend, la comunicazione con il backend avviene tramite la libreria \textbf{Axios}, un HTTP client basato su \textit{Promise} che semplifica l'invio di richieste asincrone e la gestione delle risposte. Axios è utilizzato come motore di rete centralizzato per invocare l'\gls{endpoint} di generazione AI, ma viene incapsulato all'interno di moduli di servizio (\texttt{apiClient.ts} e \texttt{aiCall.ts}) per evitare che i componenti dell'interfaccia dipendano dai dettagli di trasporto.

Nel dettaglio, l'infrastruttura basata su Axios svolge le seguenti funzioni:
\begin{itemize}
    \item \textbf{Invio delle richieste al backend}: costruisce e invia la richiesta \texttt{POST} verso \texttt{/\gls{api}/ai/generate} contenente i parametri dell'operazione, ovvero l'operazione richiesta dall'utente (\texttt{operationId}), il testo base (\texttt{text}) e l'eventuale istruzione aggiuntiva (\texttt{\gls{prompt}}, utilizzato nel \gls{Distant Writing}).
    \item \textbf{Gestione asincrona}: permette ai ViewModel del frontend di utilizzare nativamente \texttt{async}/\texttt{await} senza bloccare il thread principale dell'interfaccia utente.
    \item \textbf{Serializzazione automatica}: converte trasparentemente i \textit{payload} e gli oggetti JavaScript in stringhe JSON senza necessità di parsing manuale.
    \item \textbf{Estrazione della risposta}: riceve il JSON dal backend, ne estrae il campo utile (\texttt{generated\_text}) e lo rende immediatamente disponibile al frontend sotto forma di stringa per l'aggiornamento della UI.
    \item \textbf{Normalizzazione degli errori (\textit{Interceptor})}: centralizza la cattura di errori HTTP o di rete (es. codici \texttt{400}, \texttt{500} o timeout) e, tramite un intercettore, li trasforma in oggetti \texttt{ApiError} standardizzati, garantendo un comportamento uniforme in tutta l'applicazione.
    \item \textbf{Configurazione condivisa}: consente di definire in un unico punto (\texttt{apiClient.ts}) la \texttt{baseURL} del backend letta dall'ambiente, eventuali headers, timeout e token di cancellazione (\texttt{AbortSignal}).
\end{itemize}


\subsection{Organizzazione del codice}
Per mantenere il codice ordinato e facile da modificare, la comunicazione HTTP non viene usata direttamente nei componenti dell'interfaccia, ma è organizzata in due moduli sovrapposti.

\subsubsection{Modulo apiClient.ts}
apiClient.ts è il modulo che si occupa della comunicazione HTTP dell'applicazione, esponendo un'istanza di Axios preconfigurata e condivisa come punto di accesso centralizzato all'infrastruttura di rete.
\begin{itemize}
    \item \textbf{Problema:} Il sottosistema di networking della nostra applicazione è composto da diverse entità che devono collaborare: il client HTTP (Axios), i meccanismi di intercettazione delle richieste, il formattatore degli errori e i token di cancellazione (AbortSignal). 
    Senza un punto di accesso centralizzato, ogni componente dovrebbe istanziare e orchestrare autonomamente tutte queste entità. Questo approccio porterebbe a duplicazione del codice, comportamenti eterogenei in caso di errore e un forte accoppiamento tra la logica di presentazione e i dettagli implementativi del trasporto HTTP.
    
    \item \textbf{Applicazione:} Il modulo apiClient.ts è un'istanza di Axios preconfigurata e condivisa in tutta l'applicazione. Centralizza in un unico punto la baseURL, il timeout e gli header standard, leggendoli dalle variabili d'ambiente. Tramite un response interceptor, intercetta qualsiasi errore (di rete, HTTP o di cancellazione tramite AbortSignal) e lo trasforma in un oggetto Error omogeneo attraverso la funzione transformApiError. In questo modo il resto dell'applicazione riceve sempre errori in un formato uniforme.
\end{itemize}

\subsubsection{Modulo aiCall.ts}
...
\begin{itemize}
    \item \textbf{Problema:} Senza questo modulo, ogni ViewModel che necessita di invocare un'operazione AI dovrebbe conoscere l'URL esatto dell'\gls{endpoint}, costruire manualmente il payload JSON nel formato atteso dal backend ed estrarre il testo generato dalla struttura della risposta HTTP. Questo comporterebbe una dipendenza diretta tra la logica di presentazione e i dettagli implementativi del servizio AI: qualsiasi modifica all'\gls{endpoint} o al formato dei dati richiederebbe interventi su tutti i componenti che effettuano la chiamata.
    
    \item \textbf{Applicazione:} Il modulo astrae l'\gls{endpoint} specifico (/ai/generate) e traduce il linguaggio del frontend in quello richiesto dal server:
    \begin{itemize}
        \item \textbf{Interfaccia di dominio}: espone il metodo \texttt{executeOperation}, che accetta parametri tipizzati (\texttt{AiRequestPayload}, \texttt{AbortSignal}) e restituisce direttamente una \texttt{Promise<string>}, nascondendo la struttura raw della risposta HTTP (AiResponse.generated\_text) ai livelli superiori.
...
\subsubsection{Collaborazione tra i componenti}
L'architettura segue una struttura a strati che garantisce un alto grado di disaccoppiamento:
\begin{itemize}
    \item \textbf{ViewModel (useAiOperations.ts)}: non ha alcuna conoscenza di URL o di Axios. Invoca aiCall richiedendo l'esecuzione di un'operazione di dominio ("Riassumi questo testo"), gestendo esclusivamente la logica di stato della UI.
    \item \textbf{aiCall.ts}: conosce la struttura del servizio AI, costruisce il payload corretto e lo delega ad apiClient, specificando l'\gls{endpoint} di destinazione.
    \item \textbf{apiClient.ts}: esegue materialmente la trasmissione HTTP, gestendo i dettagli tecnici di basso livello e garantendo che qualsiasi anomalia di rete venga restituita in un formato omogeneo e gestibile.
\end{itemize}

\subsubsection{Vantaggi}
Questa architettura porta benefici misurabili in termini di qualità del software. Dal punto di vista della testabilità, la separazione in livelli distinti facilita l'esecuzione dei test dei singoli moduli e riduce l'accoppiamento tra la logica di presentazione e i dettagli implementativi della comunicazione HTTP. Dal punto di vista del disaccoppiamento tecnologico, una futura migrazione da Axios all'\gls{API} nativa \texttt{fetch} richiederebbe modifiche unicamente ad \texttt{apiClient.ts}, lasciando intatti tutti gli hook e i componenti.

Il gruppo è consapevole che, dato l'attuale numero di \gls{endpoint} esposti dal backend, il modulo apiClient e il modulo aiCall introducono una complessità non strettamente necessaria allo stato attuale del sistema. La scelta è stata deliberata e motivata dall'estensibilità futura verso nuovi \gls{endpoint} (autenticazione, salvataggio remoto dei documenti), dalla necessità di centralizzare la gestione degli errori tipici del contesto AI (timeout lunghi, risposte di provider esterni), e dalla volontà di applicare consapevolmente principi di disaccoppiamento nell'ambito del corso di Ingegneria del Software.

\subsubsection{Flusso di elaborazione}
\begin{enumerate}
\item Il ViewModel invoca \texttt{executeOperation} tramite il modulo \texttt{aiCall.ts}.
\item \texttt{aiCall.ts} delega la richiesta HTTP POST ad \texttt{apiClient}, specificando l'\gls{endpoint} \\ \texttt{/ai/generate} e il payload.
...
\item Il risultato viene restituito al client in formato JSON, estratto da \texttt{apiClient} e restituito come stringa al ViewModel.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../UMLimg/apiClient.jpg}
    \caption{Diagramma UML: Interazione tra i moduli di servizio HTTP}
    \label{fig:uml_api}
\end{figure}
