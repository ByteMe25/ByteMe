...
L'attenzione è posta sulla \textbf{qualità della documentazione}, assicurando che sia leggibile, corretta e priva di ambiguità, e sull'\gls{efficacia} del \textbf{processo di verifica}, misurando la capacità del gruppo di identificare e risolvere rischi e difetti in modo proattivo.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|c|X|c|}
        ...
        \hline
        TI-1 & \textbf{Health Check Database}: \gls{Verifica} che il backend riesca a stabilire una connessione stabile con il \gls{database} PostgreSQL interrogando l'endpoint dedicato (\texttt{/\gls{api}/test-db-connection}) ed eseguendo una query di test. & ROP-F-22 \\
        \hline
        ...
        \hline
        TI-3 & \gls{Verifica} che la chiamata di registrazione dal backend salvi correttamente e persistentemente il record del nuovo utente all'interno del \gls{database} PostgreSQL. & ROP-F-11 \\
        \hline
        TI-4 & \gls{Verifica} che l'endpoint del backend formatti correttamente il prompt richiesto e lo inoltri con successo alle \gls{API} del provider LLM, ricevendo una risposta valida. & RO-V-13 \\
        \hline
