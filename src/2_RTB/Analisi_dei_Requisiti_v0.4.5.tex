...
\hline
RO-F-3 & Deve essere presente un'area di visualizzazione del documento formattato rispetto alla sintassi Markdown che si aggiorna automaticamente & UC-1.1 \\
\hline
...
\hline
RO-F-16 & Il sistema deve permettere l'inserimento di elementi di struttura (tabelle, separatori) tramite toolbar & UC-1.8.5 \\
\hline
RO-F-17 & Il sistema deve permettere l'inserimento di liste ed elenchi nel testo tramite toolbar & UC-1.8.6 \\
\hline
RO-F-18 & Il sistema deve permettere l'inserimento di link e riferimenti tramite toolbar & UC-1.8.7 \\
\hline
RO-F-19 & L'utente può applicare più "stili" (es: grassetto, corsivo, sottolineatura) alla stessa sezione di testo & UC-1.8.* \\
\hline
RO-F-20 & L'utente può annullare una formattazione di una porzione di testo selezionato riselezionandola dalla toolbar & UC-1.8.* \\
\hline
RO-F-21 & L'utentea può estendere la formattazione di una porzione di testo selezionato riselezionandola dalla toolbar & UC-1.8.* \\
\hline
RO-F-22 & L'utente può creare sottoliste scegliendo di creare una lista dalla toolbar quando il cursore è già all'interno di una lista  & UC-1.8.6.* \\
\hline
RO-F-23 & L'utente può scegliere una combinazione di opzioni di formattazioni dalla toolbar quando non c'è alcun testo selezionato per applicarle a testo futuro & UC-1.9.* \\
\hline
RO-F-24 & Il sistema deve supportare l'inserimento e la visualizzazione di caratteri Unicode (supporto multilingua) & UC-1.0 \\
\hline
RO-F-25 & Il sistema permette di avviare la correzione grammaticale & UC-2.6 \\
\hline
RO-F-26 & Il sistema permette di inviare del testo ad un agente esterno & UC-2.0, UC-2.6, UC-2.7, UC-2.7.*, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-F-27 & L’utente può modificare, formattare e annullare le generazioni di testo & UC-2.1.* \\
\hline
RO-F-28 & Se l'utente annulla la generazione di testo, il sistema deve garantire che nessuna porzione parziale di testo generato venga inserita nel documento.
 & UC-2.1.* \\
\hline
RO-F-29 & Una volta generato, il testo dell'agente esterno viene inserito nell'editor dal sistema, in modo che tutte le funzionalità dell'editor siano applicate anche al testo generato & UC-2.0, UC-2.2, UC-2.6, UC-2.7, UC-2.7.x, UC-2.8 \\
\hline
RO-F-30 & Il sistema permette all'utente di eliminare il testo generato da un agente esterno & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5.*, UC-2.6 \\
\hline
RO-F-31 & Il sistema consente all'utente di avviare l'analisi del testo selezionato o dell'intero testo utilizzando il cappello scelto & UC-2.7, UC-2.7.* \\
\hline
RO-F-32 & Il sistema permette l'invio di un prompt scritto dall’utente per la generazione del testo tramite agente esterno & UC-2.8 \\
\hline
RO-F-33 & Il sistema permette la modifica del testo generato inserito nell'editor & UC-2.3, UC-2.4, UC-2.5.*, UC-2.8, UC-2.11 \\
\hline
RO-F-34 & Il sistema permette la modifica del prompt da inviare ad un agente esterno & UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-F-35 & L'utente deve poter accedere alle azioni sul testo tramite agente esterno in ogni momento & UC-2.0 \\
\hline
RO-F-36 & Ogni sezione che contiene testo ricevuto da un agente esterno include opzioni per eliminare o rigenerare il testo & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5.* \\
\hline
RO-F-37 & L'utente deve poter riassumere tutto o una parte selezionata del testo usando un agente esterno & UC-2.3 \\
\hline
RO-F-38 &  L'utente deve poter usare un agente esterno per riscrivere tutto o una parte selezionata del testo migliorandolo seguendo determinate specifiche come sintassi, stile e chiarezza & UC-2.4 \\
\hline
RO-F-39 & L'utente deve poter tradurre il testo usando un agente esterno in: inglese, italiano, tedesco, francese, spagnolo e cinese mandarino & UC-2.5.* \\
\hline
RO-F-40 & Se viene annullata prematuramente la generazione di testo, rimarrà ciò che è già stato generato & UC-2.3, UC-2.4, UC-2.5.* \\
\hline
RO-F-41 & L'utente può caricare/aprire un file dal proprio dispositivo all'interno dell'editor & UC-3.0, UC-3.0.0, UC-3.0.1 \\
\hline
RO-F-42 & Il sistema deve supportare il caricamento tramite dialogo di esplorazione file (file picker) & UC-3.0, UC-3.0.1 \\
\hline
RO-F-43 & Il sistema deve permettere all'utente di rimuovere il file che ha caricato (chiudendo il documento o l'editor) & UC3.0, UC-3.3 \\
\hline
RO-F-44 & Il sistema deve permettere all'utente di annullare l'operazione di caricamento file & UC-3.0 \\
\hline
RO-F-45 & Il sistema notifica l'errore se non riesce a caricare il file & UC-3.1, UC-3.2\\
\hline
RO-F-46 & Il sistema deve consentire l'esportazione del documento in formato PDF. & UC-4.0, UC-4.1\\
\hline
RO-F-47 & Il sistema deve essere in grado di convertire il documento da Markdown al formato selezionato dall'utente. & UC-4.0, UC-4.0, UC-4.1, UC-4.1.1, UC-4.1.2, UC-4.1.3, UC-4.1.4\\
\hline
RO-F-48 & Il sistema deve controllare che il range di pagine inserito dall'utente sia valido (che non superi il numero di pagine presenti o che non sia un numero negativo). & UC-4.2.1, UC-4.3.1\\
\hline
RO-F-49 & Il sistema deve permettere di poter scaricare il file nel dispositivo dell'utente. & UC-4.0\\
\hline
RO-F-50 & Il sistema deve potersi collegare a un dispositivo di stampa. & UC-4.3\\
\hline
...
\hline
RO-V-5 & I testi generati dall'agente esterno devono indicare il tipo di operazione che li ha generati & UC-2.* \\
\hline
RO-V-6 & Il testo generato da un agente esterno viene inserito nella posizione del cursore, se presente nell’editor, altrimenti in fondo al testo & UC-2.0, UC-2.2 \\
\hline
RO-V-7 & L'agente esterno deve avere istruzioni sufficienti affinché il testo riscritto sia corretto grammaticalmente, sintatticamente e semanticamente e deve riflettere la richiesta dell'utente & UC-2.4 \\
\hline
RO-V-8 & Le sezioni create per la traduzione di un testo specificheranno la lingua di partenza e di fine & UC-2.5.* \\
\hline
RO-V-9 & All'agente esterno devono essere passate delle istruzioni ottimizzate per garantire una traduzione accurata nella lingua selezionata & UC-2.5, UC-2.5.* \\
\hline
RO-V-10 & Il sistema espone la sua limitazione nell'eseguire un'azione richiesta se i dati inviati non rispettano i limiti dell'agente esterno & UC-2.9, UC-2.10, UC-2.11 \\
\hline
...
\hline
RO-V-13 & Il sistema deve collegarsi ad un LLM tramite API REST & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5, UC-2.5.x, UC-2.7, UC-2.7.x, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
...
\hline
ROP-V-6 &  testi generati dall’agente esterno devono indicare la data e l’ora della generazione & UC-2.* \\
\hline
\addlinespace[1em]
...
\hline
RO-Q-13 & Il testo creato si comporta come qualunque altro testo generato da un agente esterno: facilmente riconoscibile e sempre modificabile dall'utente & UC-2.3, UC-2.4, UC-2.5., UC-2.6, UC-2.7, UC-2.7,x, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-Q-14 & Il sistema mostra un messaggio in caso di errore durante la generazione di contenuto & UC-2.3, UC-2.4, UC-2.5.*, UC-2.6, UC-2.7, UC-2.7.*, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-Q-15 & Il sistema deve fornire un feedback visivo immediato all'utente (messaggio di errore o di successo) al termine del tentativo di caricamento file & UC-3.0, UC-3.1, UC-3.2 \\
\hline
RO-Q-16 & Il sistema deve validare il formato del file caricato e impedire il caricamento di formati non supportati & UC-3.1 \\
\hline
RO-Q-17 & Il sistema deve validare le dimensioni del file caricato e impedire il caricamento di file che superano una dimensione massima & UC-3.2 \\
\hline
RO-Q-18 & Il sistema deve impedire il caricamento di file corrotti & UC-3.0, UC-3.1\\
\hline
RO-Q-19 & La dimensione massima consentita per il caricamento di un file deve essere limitata a una soglia definita e ragionevole & UC-3.0, UC-3.2 \\
\hline
RO-Q-20 & Il sistema deve avvisare l'utente della buona riuscita o di eventuali errori durante il download. & UC-4.0, UC-4.1 \\
\hline
RO-Q-21 & Il sistema deve avvisare l’utente in caso di errore durante l’invio del documento alla stampante. & UC-4.3 \\
\hline
RO-Q-22 & Il sistema deve poter permettere di visualizzare sia il testo in lingua originale che la traduzione generata & UC-2.5, UC-2.5.x \\
\hline
