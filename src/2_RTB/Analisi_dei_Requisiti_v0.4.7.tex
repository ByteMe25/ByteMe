...
\hline
RO-F-27 & Il sistema permette di avviare la correzione grammaticale & UC-2.5.* \\
\hline
RO-F-28 & Il sistema permette di inviare del testo ad un agente esterno & UC-2.* \\
\hline
...
RO-F-30 & Se l'utente annulla la generazione di testo, il sistema deve garantire che nessuna porzione parziale di testo generato venga inserita nel documento.
 & UC-2.1.* \\
\hline
RO-F-31 & Una volta generato, il testo dell'agente esterno viene inserito nell'editor dal sistema, in modo che tutte le funzionalità dell'editor siano applicate anche al testo generato & UC-2.* \\
\hline
RO-F-32 & Il sistema permette all'utente di rimuovere tutto il testo generato da un agente esterno& UC-2.* \\
\hline
RO-F-33 & Il sistema consente all'utente di avviare l'analisi del testo selezionato o dell'intero testo utilizzando il cappello scelto & UC-2.6 \\
\hline
RO-F-34 & Il sistema permette l'invio di un prompt scritto dall’utente per la generazione del testo tramite agente esterno & UC-2.7 \\
\hline
RO-F-35 & Il sistema permette la modifica del testo generato inserito nell'editor & UC-2.* \\
\hline
RO-F-36 & Il sistema permette la modifica del prompt da inviare ad un agente esterno & UC-2.6 \\
\hline
RO-F-37 & L'utente deve poter accedere alle azioni sul testo tramite agente esterno in ogni momento & UC-2.0 \\
\hline
RO-F-38 & Ogni sezione che contiene testo ricevuto da un agente esterno include opzioni per eliminare o rigenerare il testo & UC-2.* \\
\hline
RO-F-39 & L'utente deve poter riassumere tutto o una parte selezionata del testo usando un agente esterno & UC-2.2 \\
\hline
RO-F-40 &  L'utente deve poter usare un agente esterno per riscrivere tutto o una parte selezionata del testo migliorandolo seguendo determinate specifiche come sintassi, stile e chiarezza & UC-2.3 \\
\hline
RO-F-41 & L'utente deve poter tradurre il testo usando un agente esterno in: inglese, italiano, tedesco, francese, spagnolo e cinese mandarino & UC-2.4.* \\
\hline
RO-F-42 & Se viene annullata prematuramente la generazione di testo, rimarrà ciò che è già stato generato & UC-2.* \\
\hline
RO-F-43 & L'agente esterno può generare testo basandosi su prompt inviatogli dall'utente & UC-2.7.* \\
\hline
RO-F-44 & L'utente può rigenerare la risposta alla propria query nel caso fosse insoddisfatto con l'output dell'agente esterno & UC-2.1.3 \\
\hline
RO-F-45 & L'utente può visualizzare il testo scritto da se stesso, senza quello generato da un agente esterno & UC-2.1.4 \\
\hline
RO-F-46 & L'utente può visualizzare il testo generato da un agente esterno, senza quello che ha scritto dall'utente & UC-2.1.5 \\
\hline
RO-F-47 & L'utente può caricare/aprire un file dal proprio dispositivo all'interno dell'editor & UC-3.0, UC-3.0.0, UC-3.0.1 \\
\hline
RO-F-48 & Il sistema deve supportare il caricamento tramite dialogo di esplorazione file (file picker) & UC-3.0, UC-3.0.1 \\
\hline
RO-F-49 & Il sistema deve permettere all'utente di rimuovere il file che ha caricato (chiudendo il documento o l'editor) & UC3.0, UC-3.3 \\
\hline
RO-F-50 & Il sistema deve permettere all'utente di annullare l'operazione di caricamento file & UC-3.0 \\
\hline
RO-F-51 & Il sistema notifica l'errore se non riesce a caricare il file & UC-3.1, UC-3.2\\
\hline
RO-F-52 & Il sistema deve consentire l'esportazione del documento in formato PDF. & UC-4.0, UC-4.1\\
\hline
RO-F-53 & Il sistema deve essere in grado di convertire il documento da Markdown al formato selezionato dall'utente. & UC-4.0, UC-4.0, UC-4.1, UC-4.1.1, UC-4.1.2, UC-4.1.3, UC-4.1.4\\
\hline
RO-F-54 & Il sistema deve controllare che il range di pagine inserito dall'utente sia valido (che non superi il numero di pagine presenti o che non sia un numero negativo). & UC-4.2.1, UC-4.3.1\\
\hline
RO-F-55 & Il sistema deve permettere di poter scaricare il file nel dispositivo dell'utente. & UC-4.0\\
\hline
RO-F-56 & Il sistema deve potersi collegare a un dispositivo di stampa. & UC-4.3\\
\hline
...
\hline
ROP-F-2 & Il sistema separa visivamente il testo generato e quello utente & UC-2.* \\
\hline
ROP-F-3 & La sezione viene colorata in base al tipo di operazione che viene fatta & UC-2.0 \\
\hline
...
\hline
ROP-F-19 & Il sistema deve permettere ad un utente autenticato di selezionare un file dall'elenco e aprirlo & UC-5.3 \\
\hline
...
\hline
ROP-F-21 & Il sistema deve permettere all'utente l'apertura di un file salvato e visibile all'utente & UC-5.3 \\
\hline
...
\hline
ROP-F-24 & L'utente può effettuare il logout in ogni momento e terminare la propria sessione & UC-5.5 \\
\hline
...
\hline
RO-V-6 & Il testo generato da un agente esterno viene inserito nella posizione del cursore, se presente nell’editor, altrimenti in fondo al testo & UC-2.* \\
\hline
...
\hline
ROP-V-7 & I testi generati dall’agente esterno devono indicare il tipo di operazione che li ha generati & UC-2.* \\
\hline
\addlinespace[1em]
...
\hline
RO-Q-13 & Il testo creato si comporta come qualunque altro testo generato da un agente esterno: facilmente riconoscibile e sempre modificabile dall'utente & UC-2.* \\
\hline
RO-Q-14 & Il sistema mostra un messaggio in caso di errore durante la generazione di contenuto & UC-2.* \\
\hline
