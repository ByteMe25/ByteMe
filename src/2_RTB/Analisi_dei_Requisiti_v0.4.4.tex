...
\hline
RO-F-22 & L’utente può modificare, formattare e annullare le generazioni di testo & UC-2.1.* \\
\hline
RO-F-23 & Se l'utente annulla la generazione di testo, il sistema deve garantire che nessuna porzione parziale di testo generato venga inserita nel documento.
 & UC-2.1.* \\
\hline
RO-F-24 & Una volta generato, il testo dell'agente esterno viene inserito nell'editor dal sistema, in modo che tutte le funzionalità dell'editor siano applicate anche al testo generato & UC-2.0, UC-2.2, UC-2.6, UC-2.7, UC-2.7.x, UC-2.8 \\
\hline
RO-F-25 & Il sistema permette all'utente di eliminare il testo generato da un agente esterno & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5.*, UC-2.6 \\
\hline
RO-F-26 & Il sistema consente all'utente di avviare l'analisi del testo selezionato o dell'intero testo utilizzando il cappello scelto & UC-2.7, UC-2.7.* \\
\hline
RO-F-27 & Il sistema permette l'invio di un prompt scritto dall’utente per la generazione del testo tramite agente esterno & UC-2.8 \\
\hline
RO-F-28 & Il sistema permette la modifica del testo generato inserito nell'editor & UC-2.3, UC-2.4, UC-2.5.*, UC-2.8, UC-2.11 \\
\hline
RO-F-29 & Il sistema permette la modifica del prompt da inviare ad un agente esterno & UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-F-30 & L'utente deve poter accedere alle azioni sul testo tramite agente esterno in ogni momento & UC-2.0 \\
\hline
RO-F-31 & Ogni sezione che contiene testo ricevuto da un agente esterno include opzioni per eliminare o rigenerare il testo & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5.* \\
\hline
RO-F-32 & L'utente deve poter riassumere tutto o una parte selezionata del testo usando un agente esterno & UC-2.3 \\
\hline
RO-F-33 &  L'utente deve poter usare un agente esterno per riscrivere tutto o una parte selezionata del testo migliorandolo seguendo determinate specifiche come sintassi, stile e chiarezza & UC-2.4 \\
\hline
RO-F-34 & L'utente deve poter tradurre il testo usando un agente esterno in: inglese, italiano, tedesco, francese, spagnolo e cinese mandarino & UC-2.5.* \\
\hline
RO-F-35 & Se viene annullata prematuramente la generazione di testo, rimarrà ciò che è già stato generato & UC-2.3, UC-2.4, UC-2.5.* \\
\hline
RO-F-36 & L'utente può caricare/aprire un file dal proprio dispositivo all'interno dell'editor & UC-3.0, UC-3.0.0, UC-3.0.1 \\
\hline
RO-F-37 & Il sistema deve supportare il caricamento tramite dialogo di esplorazione file (file picker) & UC-3.0, UC-3.0.1 \\
\hline
RO-F-38 & Il sistema deve permettere all'utente di rimuovere il file che ha caricato (chiudendo il documento o l'editor) & UC3.0, UC-3.3 \\
\hline
RO-F-39 & Il sistema deve permettere all'utente di annullare l'operazione di caricamento file & UC-3.0 \\
\hline
RO-F-40 & Il sistema notifica l'errore se non riesce a caricare il file & UC-3.1, UC-3.2\\
\hline
RO-F-41 & Il sistema deve consentire l'esportazione del documento in formato PDF. & UC-4.0, UC-4.1\\
\hline
RO-F-42 & Il sistema deve essere in grado di convertire il documento da Markdown al formato selezionato dall'utente. & UC-4.0, UC-4.0, UC-4.1, UC-4.1.1, UC-4.1.2, UC-4.1.3, UC-4.1.4\\
\hline
RO-F-43 & Il sistema deve controllare che il range di pagine inserito dall'utente sia valido (che non superi il numero di pagine presenti o che non sia un numero negativo). & UC-4.2.1, UC-4.3.1\\
\hline
RO-F-44 & Il sistema deve permettere di poter scaricare il file nel dispositivo dell'utente. & UC-4.0\\
\hline
RO-F-45 & Il sistema deve potersi collegare a un dispositivo di stampa. & UC-4.3\\
\hline
...
\hline
RO-Q-5 & Il sistema deve intercettare e gestire gli errori di comunicazione con il servizio LLM. & UC-2.* \\
\hline
RO-Q-6 & Le opzioni, relative alle operazioni con agente esterno, non devono sparire in maniera poco intuitiva muovendo il mouse & UC-2.0 \\
\hline
RO-Q-7 & Il testo creato si comporta come qualunque altro testo generato da un agente esterno: facilmente riconoscibile e sempre modificabile dall'utente & UC-2.3, UC-2.4, UC-2.5., UC-2.6, UC-2.7, UC-2.7,x, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
...
\hline
RO-Q-14 & Il sistema deve avvisare l'utente della buona riuscita o di eventuali errori durante il download. & UC-4.0, UC-4.1 \\
\hline
RO-Q-15 & Il sistema deve avvisare l’utente in caso di errore durante l’invio del documento alla stampante. & UC-4.3 \\
\hline
RO-Q-16 & Il sistema deve poter permettere di visualizzare sia il testo in lingua originale che la traduzione generata & UC-2.5, UC-2.5.x \\
\hline
