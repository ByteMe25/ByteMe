...
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.3\textwidth}|c|}
        \hline
        \textbf{Versione} & \textbf{Data} & \textbf{Autore} & \textbf{Descrizione} & \textbf{Verifica} \\
        \hline
        0.3.7 & 15/01/2026 & Joseph Grant & Sistemate estensioni macroarea 1 e sistemato UC 3.0  &  \\
        \hline
        0.3.6 & 12/01/2026 & Matteo Cuogo & Estensione e correzzione casi d'uso macroarea 1 & Joseph Grant\\
        \hline
...
\subsubsection{UC-1.0.1 - La connessione internet si interrompe}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.0.2 - L'editor non risponde}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.1.1 - L'anteprima non si aggiorna correttamente}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\begin{itemize}
    ...
    \item \textbf{Postcondizioni}: Il sistema non fa niente
    \item \textbf{Scenario}:
        \begin{itemize}
            \item L'utente tenta di selezionare l'opzione "Annulla"
            \item Il comando undo non fa niente
        \end{itemize}
\end{itemize}
...
\subsubsection{UC-1.7.1 - L'utente non ha effettuato modifiche ripristinabili}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    ...
    \item \textbf{Scenario}:
        \begin{itemize}
            ...
            \item Il comando redo non fa niente
        \end{itemize}
\end{itemize}
...
\subsubsection{UC-1.8 - Modifica del testo selezionato tramite barra degli strumenti}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.8.0.1 - L'utente vuole applicare una nuova formattazione a un testo già formattato}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    ...
    \item \textbf{Postcondizioni}: Il sistema applica la nuova formattazione sopra alla formattazione esistente
    \item \textbf{Scenario}:
        \begin{itemize}
            \item L'utente seleziona il testo già formattato
...
\subsubsection{UC-1.8.0.2 - Il testo ha già una specifica formattazione e l'utente la applica di nuovo}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: L'utente riapplica la stessa formattazione a del testo
...
\subsubsection{UC-1.8.0.3 - Il testo selezionato è solo parzialmente formattato}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: L'utente applica la stessa formattazione a una porzione del testo che contiente parzialmente la stessa formattazione
...
\subsubsection{UC-1.8.4 - Applicazione intestazioni}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.8.5 - Applicazione struttura documento}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.8.6 - Applicazione inserimento liste ed elenchi}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: Creazione di liste ordinate e non ordinate nel documento dal testo selezionato
    \item \textbf{Precondizioni}: È presente testo selezionato nell’editor
...
\subsubsection{UC-1.8.6.1 - L'utente vuole inserire una sotto-lista}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    ...
    \item \textbf{Scenario}:
        \begin{itemize}
            \item L'utente inizializza una lista indentata dentro ad un'altra tramite
...
\subsubsection{UC-1.8.7 - Applicazione inserimento link e riferimenti}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.9 - Impostare formattazioni al testo futuro}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: Applicazione formattazioni al testo futuro tramite barra degli strumenti
    \item \textbf{Precondizioni}: L’utente ha accesso agli strumenti di formattazione
    \item \textbf{Postcondizioni}: Il testo futuro viene formattato in base alla selezione della formattazione dell’utente
    \item \textbf{Scenario principale}:
        \begin{itemize}
...
\subsubsection{UC-1.9.0.1 - L'utente vuole scrivere con multiple opzioni di formattazione}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    ...
    \item \textbf{Scenario}:
        \begin{itemize}
            \item L'utente abilita più opzioni di formattazione per il testo futuro
...
\subsubsection{UC-1.10 - Modalità di visualizzazione solo editor}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: Visualizzazione esclusiva dell’editor di testo
    ...
    \item \textbf{Scenario principale}:
        \begin{itemize}
            \item L'utente seleziona la modalità di visualizzazione:
                    \begin{itemize}
...
\subsubsection{UC-1.11 - Modalità di visualizzazione solo documento formattato}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: Visualizzazione esclusiva del documento formattato
    \item \textbf{Precondizioni}: La modalità di visualizzazione comprende solo l’editor oppure è in modalità split view
    \item \textbf{Postcondizioni}: Cambia la modalità di visualizzazione del documento tramite il pulsante “visualizzazione documento”
    \item \textbf{Scenario principale}:
        \begin{itemize}
            \item L'utente seleziona la modalità di visualizzazione:
                    \begin{itemize}
...
\subsubsection{UC-1.12 - Modalità di visualizzazione split view}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: Visualizzazione split view dell’editor e del documento
    \item \textbf{Precondizioni}: La modalità di visualizzazione comprende solo l’editor oppure solo il documento

    \item \textbf{Postcondizioni}: Cambia la modalità di visualizzazione split view tramite il pulsante “visualizzazione split view”
    \item \textbf{Scenario principale}:
        \begin{itemize}
            \item L'utente seleziona la modalità di visualizzazione:
                    \begin{itemize}
...
\subsubsection{UC-1.13 - Ricerca nel testo}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.13.1 - La ricerca non ha occorrenze nel testo}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    ...
    \item \textbf{Scenario}:
        \begin{itemize}
            ...
            \item Il sistema informa l'utente che nessuna occorrenza è stata trovata
        \end{itemize}
\end{itemize}
...
\hline
RO-F-20 & Il sistema permette di avviare la correzione grammaticale & UC-2.6 \\
\hline
RO-F-21 & Il sistema permette di inviare del testo ad un agente esterno & UC-2.0, UC-2.6, UC-2.7, UC-2.7.*, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-F-22 & Una volta generato, il testo dell'agente esterno viene inserito nell'editor dal sistema, in modo che tutte le funzionalità dell'editor siano applicate anche al testo generato & UC-2.0, UC-2.2, UC-2.6, UC-2.7, UC-2.7.x, UC-2.8 \\
\hline
RO-F-23 & Il sistema permette all'utente di eliminare il testo generato da un agente esterno & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5.*, UC-2.6 \\
\hline
RO-F-24 & Il sistema consente all'utente di avviare l'analisi del testo selezionato o dell'intero testo utilizzando il cappello scelto & UC-2.7, UC-2.7.* \\
\hline
RO-F-25 & Il sistema permette l'invio di un prompt scritto dall’utente per la generazione del testo tramite agente esterno & UC-2.8 \\
\hline
RO-F-26 & Il sistema permette la modifica del testo generato inserito nell'editor & UC-2.3, UC-2.4, UC-2.5.*, UC-2.8, UC-2.11 \\
\hline
RO-F-27 & Il sistema permette la modifica del prompt da inviare ad un agente esterno & UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-F-28 & L'utente deve poter accedere alle azioni sul testo tramite agente esterno in ogni momento & UC-2.0 \\
\hline
RO-F-29 & Ogni sezione che contiene testo ricevuto da un agente esterno include opzioni per eliminare o rigenerare il testo & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5.* \\
\hline
RO-F-30 & L'utente deve poter riassumere tutto o una parte selezionata del testo usando un agente esterno & UC-2.3 \\
\hline
RO-F-31 &  L'utente deve poter usare un agente esterno per riscrivere tutto o una parte selezionata del testo migliorandolo seguendo determinate specifiche come sintassi, stile e chiarezza & UC-2.4 \\
\hline
RO-F-32 & L'utente deve poter tradurre il testo usando un agente esterno in: inglese, italiano, tedesco, francese, spagnolo e cinese mandarino & UC-2.5.* \\
\hline
RO-F-33 & Se viene annullata prematuramente la generazione di testo, rimarrà ciò che è già stato generato & UC-2.3, UC-2.4, UC-2.5.* \\
\hline
RO-F-34 & L'utente può caricare/aprire un file dal proprio dispositivo all'interno dell'editor & UC-3.0, UC-3.0.0, UC-3.0.1 \\
\hline
RO-F-35 & Il sistema deve supportare il caricamento tramite dialogo di esplorazione file (file picker) & UC-3.0, UC-3.0.1 \\
\hline
RO-F-36 & Il sistema deve permettere all'utente di rimuovere il file che ha caricato (chiudendo il documento o l'editor) & UC3.0, UC-3.3 \\
\hline
RO-F-37 & Il sistema deve permettere all'utente di annullare l'operazione di caricamento file & UC-3.0 \\
\hline
RO-F-38 & Il sistema notifica l'errore se non riesce a caricare il file & UC-3.1, UC-3.2\\
\hline
RO-F-39 & Il sistema deve consentire l'esportazione del documento in formato PDF. & UC-4.0, UC-4.1\\
\hline
RO-F-40 & Il sistema deve essere in grado di convertire il documento da Markdown al formato selezionato dall'utente. & UC-4.0, UC-4.0, UC-4.1, UC-4.1.1, UC-4.1.2, UC-4.1.3, UC-4.1.4\\
\hline
RO-F-41 & Il sistema deve controllare che il range di pagine inserito dall'utente sia valido (che non superi il numero di pagine presenti o che non sia un numero negativo). & UC-4.2.1, UC-4.3.1\\
\hline
RO-F-42 & Il sistema deve permettere di poter scaricare il file nel dispositivo dell'utente. & UC-4.0\\
\hline
RO-F-43 & Il sistema deve potersi collegare a un dispositivo di stampa. & UC-4.3\\
\hline
...
\hline
RD-F-6 & Il sistema deve consentire l'esportazione del documento in formato MD. & UC-4.1.1 \\
\hline
RD-F-7 & Il sistema deve consentire l'esportazione del documento in formato HTML. & UC-4.1.2 \\
\hline
RD-F-8 & Il sistema deve consentire l'esportazione del documento in formato ODT. & UC-4.1.3 \\
\hline
RD-F-9 & Il sistema deve consentire l'esportazione del documento in formato DOCX. & UC-4.1.4 \\
\hline
RD-F-10 & L'utente deve poter scegliere se esportare l'intero documento, un range di pagine o una quantità selezionata di pagine. & UC-4.2, UC-4.2.1\\
\hline
RD-F-11 & Il sistema deve permettere all’utente di scegliere se stampare tutto il documento o una quantità selezionata di pagine. & UC-4.3, UC-4.3.1\\
\hline
ROP-F-1 & L'utente può ricercare parole nel testo attraverso l'editor & UC-1.10 \\
\hline
ROP-F-2 & Il sistema separa visivamente il testo generato e quello utente & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5, UC-2.5.* \\
\hline
...
\hline
ROP-F-5 & Il sistema permette di inserire un grafico generato, con dati presenti nel testo, scelto tra diversi tipi disponibili all'utente & UC-2.9 \\
\hline
ROP-F-6 & Il sistema permette di inserire una tabella generata con dati presenti nel testo & UC-2.9 \\
\hline
ROP-F-7 & Il sistema permette di inserire una mappa concettuale generata con dati presenti nel testo & UC-2.10 \\
\hline
ROP-F-8 & Il sistema permette di inserire una formula matematica generata con dati presenti nel testo & UC-2.11 \\
\hline
...
\hline
RO-V-4 & L’agente esterno applica modifiche esclusivamente al testo selezionato. Un contesto aggiuntivo può essere fornito solo a scopo interpretativo e non deve essere alterato. & UC-2.0 \\
\hline
RO-V-5 & Il testo generato da un agente esterno viene inserito nella posizione del cursore, se presente nell’editor, altrimenti in fondo al testo & UC-2.0, UC-2.2 \\
\hline
RO-V-6 & L'agente esterno deve avere istruzioni sufficienti affinché il testo riscritto sia corretto grammaticalmente, sintatticamente e semanticamente e deve riflettere la richiesta dell'utente & UC-2.4 \\
\hline
RO-V-7 & Le sezioni create per la traduzione di un testo specificheranno la lingua di partenza e di fine & UC-2.5.* \\
\hline
RO-V-8 & All'agente esterno devono essere passate delle istruzioni ottimizzate per garantire una traduzione accurata nella lingua selezionata & UC-2.5, UC-2.5.* \\
\hline
RO-V-9 & Il sistema espone la sua limitazione nell'eseguire un'azione richiesta se i dati inviati non rispettano i limiti dell'agente esterno & UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-V-10 & Il caricamento e la lettura del file devono avvenire localmente tramite le API del browser (es. File API), senza inviare il contenuto a un server & UC-3.* \\
\hline
RO-V-11 & L'operazione di esportazione o stampa non deve alterare o chiudere il documento originale nell'editor. & Tutti gli UC-4 \\
\hline
RO-V-12 & Il sistema deve collegarsi ad un LLM tramite API REST & UC-2.0, UC-2.2, UC-2.3, UC-2.4, UC-2.5, UC-2.5.x, UC-2.7, UC-2.7.x, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
...
\hline
ROP-V-5 & Se il testo è già nella lingua da tradurre, l'utente viene avvisato e l'operazione viene interrotta & UC-2.5.* \\
\hline
\addlinespace[1em]
...
\hline
RO-Q-4 & L'agente esterno deve rispondere sempre in tempi brevi, o restituire errore & UC-2.* \\
\hline
RO-Q-5 & Le opzioni, relative alle operazioni con agente esterno, non devono sparire in maniera poco intuitiva muovendo il mouse & UC-2.0 \\
\hline
RO-Q-6 & Il testo creato si comporta come qualunque altro testo generato da un agente esterno: facilmente riconoscibile e sempre modificabile dall'utente & UC-2.3, UC-2.4, UC-2.5., UC-2.6, UC-2.7, UC-2.7,x, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-Q-7 & Il sistema mostra un messaggio in caso di errore durante la generazione di contenuto & UC-2.3, UC-2.4, UC-2.5.*, UC-2.6, UC-2.7, UC-2.7.*, UC-2.8, UC-2.9, UC-2.10, UC-2.11 \\
\hline
RO-Q-8 & Il sistema deve fornire un feedback visivo immediato all'utente (messaggio di errore o di successo) al termine del tentativo di caricamento file & UC-3.0, UC-3.1, UC-3.2 \\
\hline
RO-Q-9 & Il sistema deve validare il formato del file caricato e impedire il caricamento di formati non supportati & UC-3.1 \\
\hline
RO-Q-10 & Il sistema deve validare le dimensioni del file caricato e impedire il caricamento di file che superano una dimensione massima & UC-3.2 \\
\hline
RO-Q-11 & Il sistema deve impedire il caricamento di file corrotti & UC-3.0, UC-3.1\\
\hline
RO-Q-12 & La dimensione massima consentita per il caricamento di un file deve essere limitata a una soglia definita e ragionevole & UC-3.0, UC-3.2 \\
\hline
RO-Q-13 & Il sistema deve avvisare l'utente della buona riuscita o di eventuali errori durante il download. & UC-4.0, UC-4.1 \\
\hline
RO-Q-14 & Il sistema deve avvisare l’utente in caso di errore durante l’invio del documento alla stampante. & UC-4.3 \\
\hline
RO-Q-15 & Il sistema deve poter permettere di visualizzare sia il testo in lingua originale che la traduzione generata & UC-2.5, UC-2.5.x \\
\hline
RD-Q-1 & I messaggi di errore (per formato errato, file troppo grande o corrotto) devono essere esplicativi e suggerire all'utente come procedere & UC-3.0, UC-3.1, UC-3.2 \\
\hline
...
\hline
RD-Q-4 & Il sistema deve fornire una visualizzazione in anteprima del documento nel formato selezionato prima di procedere al download. & UC-4.0, UC-4.1 \\
\hline
RD-Q-5 & Il sistema deve permettere all'utente di scegliere il nome del file prima di scaricare il documento. & UC-4.0, UC-4.1 \\
\hline
RD-Q-6 & Il sistema deve permettere la visualizzazione in anteprima del documento prima dell'invio del comando di stampa. & UC-4.3 \\
\hline
