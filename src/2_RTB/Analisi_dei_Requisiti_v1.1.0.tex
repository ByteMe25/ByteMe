...
\hline
\endhead
RO-F-1 & Il sistema deve fornire un'area di testo per l'inserimento e la modifica di caratteri alfanumerici e la compilazione Markdown & UC-1.0 \\
\hline
...
\hline
RO-F-7 & Il sistema deve permettere all'utente di incollare un testo nell'area di testo & UC-1.4 \\
\hline
...
\hline
RO-F-29 & Una volta generato e confermato, il testo dell'agente esterno viene inserito nell'area di testo dal sistema, in modo che tutte le funzionalità dell'editor siano applicabili anche al testo generato & UC-2.*  \\
\hline
RO-F-30 & Ogni generazione di testo dell'agente esterno viene inclusa in un registro delle generazioni, anche quelle annullate dall'utente & UC-2.1.4\\
\hline
RO-F-31 & Il testo generato viene prima mostrato come proposta: ogni proposta viene generata separata dal testo dell'utente & UC-2.1  \\
\hline
RO-F-32 & Il testo in stato di proposta include le opzioni di conferma, annullamento o rigenerazione & UC-2.1 \\
\hline
RO-F-33 & Il sistema non permette la modifica del testo generato in stato di proposta & UC-2.* \\
\hline
RO-F-34 & Il testo generato, confermato e inserito nell'area di testo può essere modificato tramite sintassi Markdown \\
\hline
...
\hline
RO-F-37 & Una proposta annullata comporta la rimozione del testo generato dalla sezione di proposta e la rimozione della sezione stessa & UC-2.1.5 \\
\hline
...
\hline
RO-F-52 & Il sistema deve consentire l'esportazione del documento in formato MD & UC-4.0.1\\
\hline
RO-F-53 & Il sistema deve essere in grado di convertire il documento da Markdown al formato selezionato dall'utente & UC-4.0 \\
\hline
RO-F-54 & Il sistema deve controllare che il range di pagine inserito dall'utente sia valido (che non superi il numero di pagine presenti o che non sia un numero negativo) & UC-4.1.* \\
\hline
RO-F-55 & Il sistema deve permettere di poter scaricare il file nel dispositivo dell'utente & UC-4.0\\
\hline
RO-F-56 & Il sistema deve potersi collegare a un dispositivo di stampa & UC-4.2\\
\hline
...
\hline
RD-F-4 & Il sistema deve offrire opzioni per le operazioni comuni sui file come carica nuovo, stampa, chiudi & UC-3.0, UC-3.3, UC-4.0, UC-4.2\\
\hline
...
\hline
RD-F-6 & Il sistema deve consentire l'esportazione del documento in formato PDF & UC-4.0.2 \\
\hline
...
\hline
RD-F-10 & L'utente deve poter scegliere se esportare l'intero documento o un range di pagine selezionate & UC-4.1.*\\
\hline
RD-F-11 & Il sistema deve permettere all'utente di scegliere se stampare tutto il documento o un range di pagine selezionate & UC-4.2.*\\
\hline
...
\hline
ROP-F-10 & Se le credenziali sono errate il sistema non permette all'utente di autenticarsi & UC-5.0.* \\
\hline
...
\hline
ROP-F-17 & Il sistema deve terminare la sessione se un account viene eliminato & UC-5.2 \\
\hline
ROP-F-18 & Il sistema deve permettere ad un utente di vedere i file che ha salvato & UC-5.3 \\
\hline
ROP-F-19 & Il sistema deve permettere ad un utente autenticato di selezionare un file dall'elenco di quelli salvati e aprirlo & UC-5.3 \\
\hline
...
\hline
RO-V-4 & Il sistema deve integrarsi con il servizio LLM di OpenAI tramite le relative API REST.  & Capitolato \\
\hline
RO-V-5 & La password degli utenti registrati deve essere lunga almeno 8 caratteri e contenere almeno un carattere speciale e un numero. & UC-5.1 \\
\hline
RO-V-6 & Il sistema deve essere sviluppato rispettando le Norme 
di Progetto del gruppo ByteMe. & Norme di Progetto v1.0.0 \\
\hline
RO-V-7 & Il sistema deve essere verificato secondo il Piano di Qualifica prima di ogni rilascio. & Piano di Qualifica v1.0.0 \\
\hline
RD-V-1 & Il sistema deve supportare l'importazione dei formati Markdown (.md, .markdown) e testo semplice (.txt). & UC-3.0 \\
\hline
\addlinespace[1em]
...
\hline
UC-1.0 & Utilizzo editor Markdown & RO-F-1, RO-F-26 \\
\hline
UC-1.1 & Visualizzazione anteprima & RO-F-2, RO-F-3 \\
\hline
UC-1.2 & Selezione del testo & RO-F-4, RO-F-5 \\
\hline
UC-1.3 & Copiare il testo & RO-F-6 \\
\hline
...
\hline
Funzionali & 56 & 11 & 25 & 92 \\
\hline
Vincolo & 13 & 1 & 6 & 20 \\
\hline
Qualità & 21 & 5 & 5 & 31 \\
\hline
\textbf{Totale} & 90 & 17 & 36 & 143\\
\hline
\end{tabular}
