...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|>{\raggedright\arraybackslash}p{0.4\textwidth}|c|}
        ...
        \hline
        0.1.5 & 21/02/2026 & Giulia Barzon & Aggiunti test analisi dinamica &  \\
        \hline
...
L'analisi dinamica del software \textit{Second Brain} viene condotta attraverso l'esecuzione di test strutturati su diversi livelli di granularità, secondo il modello a V del ciclo di vita del software. L'obiettivo è verificare che il sistema soddisfi i requisiti funzionali e non funzionali, garantendo affidabilità, correttezza e usabilità del prodotto finale.

\paragraph{Priorità di Testing (per la fase RTB)}
Durante lo sviluppo del Proof of Concept (PoC), l'analisi dinamica si concentra sulla verifica della fattibilità tecnologica e sull'integrazione tra i componenti. Le priorità sono così definite:
\begin{itemize}
    \item \textbf{Alta}: Test di Sistema (funzionalità AI core) e Test di Integrazione (API Auth + AI, Backend-Database).
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|c|c|X|p{2.5cm}|p{2.5cm}|}
        ...
        \hline
        TU-1 & \texttt{genera\_hash()} & Verifica generazione hash PBKDF2 con salt casuale. & pwd: "test123" & Stringa formato \texttt{salt\$hash} \\
        \hline
        TU-2 & \texttt{verifica\_pwd()} & Verifica confronto con una password corretta. & pwd: "test123", hash valido & \texttt{True} \\
        \hline
        TU-3 & \texttt{verifica\_pwd()} & Verifica rifiuto con una password errata. & pwd: "wrong", hash valido & \texttt{False} \\
        \hline
        TU-4 & \texttt{verifica\_pwd()} & Gestione delle eccezioni per hash malformati o nulli. & hash malformato & \texttt{False} \\
        \hline
    \end{tabularx}
...
\textbf{Obiettivo}: Verificare che i diversi moduli del sistema (Frontend, Backend, Database e LLM esterni) comunichino correttamente tra loro attraverso le rispettive interfacce e API.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|c|X|c|}
        ...
        \hline
        TI-1 & \textbf{Health Check Database}: Verifica che il backend riesca a stabilire una connessione stabile con il database PostgreSQL interrogando l'endpoint dedicato (\texttt{/api/test-db-connection}) ed eseguendo una query di test. & ROP-F-22 \\
        \hline
        TI-2 & \textbf{Health Check LLM}: Verifica in fase di avvio dell'applicazione che le variabili d'ambiente (API Key e Base URL) siano correttamente caricate e che il client verso il provider esterno venga inizializzato con successo. & RO-V-13 \\
        \hline
        TI-3 & Verifica che la chiamata di registrazione dal backend salvi correttamente e persistentemente il record del nuovo utente all'interno del database PostgreSQL. & ROP-F-11 \\
        \hline
        TI-4 & Verifica che l'endpoint del backend formatti correttamente il prompt richiesto e lo inoltri con successo alle API del provider LLM, ricevendo una risposta valida. & RO-V-13 \\
        \hline
        TI-5 & Verifica che la funzione frontend \texttt{loginUtente()} componga correttamente il body JSON della richiesta HTTP POST e gestisca adeguatamente la risposta del backend (es. token di sessione o errore). & ROP-F-9 \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|c|X|c|}
        ...
        \hline
        TS-3 & \textbf{Flusso Editor e I/O}: L'utente scrive testo formattato in Markdown nell'editor, verifica che la sintassi venga riconosciuta, ed esegue il salvataggio (download) del file in formato .md sul proprio dispositivo. & UC-1.0, UC-4.0 \\
        \hline
    \end{tabularx}
