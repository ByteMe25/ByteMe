...
\begin{titlepage}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{5cm}|}
            ...
            \hline
            \textbf{Verifica} & Giulia Barzon, Elisa Marchioro, Matteo Cuogo \\
            \hline
            \textbf{Approvazione} & \\
            \hline
...
\begin{longtable}[H]{|c|c|c|>{\raggedright\arraybackslash}p{0.3\textwidth}|c|}
    ...
    \label{tab:versionamento} \\
        ...
        \hline
        1.2.0 & 5/03/2026 & Giulia Barzon & Aggiunta tabella tracciamento UC & \\
        \hline
        1.1.0 & 4/03/2026 & Giulia Barzon & Correzioni a requisiti e casi d'uso & \\
        \hline
...
Per evitare ambiguità, i termini tecnici usati nel documento vengono definiti in modo più approfondito nel documento "Glossario v.1.0.0" che si trova nella repository del gruppo nella cartella "Documenti\textunderscore interni".
...
\begin{itemize}
    \item Norme di Progetto v1.0.0 - Documento interno del gruppo ByteMe
...
\begin{itemize}
    ...
    \item \textbf{Descrizione}: L'utente scrive testo nell'area di editing dell'editor. Il sistema supporta la sintassi Markdown standard (CommonMark), interpretando il testo inserito e visualizzandolo formattato nell'anteprima in tempo reale
...
\subsubsection{UC-1.8.0.1 - Testo già parzialmente formattato}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-1.8.1 - Rimozione formattazione tramite toolbar}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\begin{itemize}
    ...
    \item \textbf{Scenario principale}:
        \begin{itemize}
            ...
            \item Il sistema applica la sintassi Markdown corrispondente, anche in caso di multiple formattazioni
...
\begin{itemize}
    ...
    \item \textbf{Postcondizioni}: Viene generato un nuovo testo basato sul prompt e inserito nell’editor in seguito alla conferma dell'utente
    \item \textbf{Scenario principale}:
    \begin{itemize}
...
\subsubsection{UC-3.0.1 - Caricamento file Markdown}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
    \item \textbf{Descrizione}: L'utente carica un file in formato Markdown (\texttt{.md}, \texttt{.markdown}) nell'editor
    \item \textbf{Precondizioni}: L'utente dispone di un file in formato Markdown
    \item \textbf{Postcondizioni}: Il contenuto del file Markdown viene caricato nell'editor
    \item \textbf{Scenario principale}:
        \begin{itemize}
            \item L'utente seleziona un file con estensione \texttt{.md} o \texttt{.markdown}
...
\subsubsection{UC-3.0.3 - Opzione trascina file}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\subsubsection{UC-3.0.4 - Opzione esplora file}
\begin{itemize}
    \item \textbf{Attore principale}: Utente generico
...
\begin{figure}[H]
    ...
    \caption{Diagramma dei casi d'uso UC-3.0, 3.0.*}
    \label{fig:uc3.0}
\end{figure}
...
\subsubsection{UC-5.3 - Apertura di un file salvato}
\begin{itemize}
    \item \textbf{Attore principale}: Utente autenticato
    \item \textbf{Descrizione}: L'utente autenticato può aprire un documento precedentemente salvato nel proprio account
    ...
    \item \textbf{Scenario principale}:
        \begin{itemize}
            ...
            \item Il sistema mostra i documenti salvati dall'utente autenticato
...
% OBBLIGATORI
% OBBLIGATORI
RO-F-1 & Il sistema deve fornire un'area di testo per l'inserimento e la modifica del testo con supporto alla sintassi Markdown standard (CommonMark). & UC-1.0 \\
\hline
...
\hline
RO-F-4 & L'editor deve compilare la sintassi Markdown in tempo reale. & UC-1.1 \\
\hline
RO-F-5 & Deve essere presente un'area di visualizzazione del documento formattato rispetto alla sintassi Markdown. & UC-1.1 \\
\hline
...
\hline
RO-F-16 & Nell'editor è presente una barra degli strumenti (toolbar) per applicare formattazioni Markdown senza scrivere manualmente la sintassi. & UC-1.8.0, UC-1.8.* \\
\hline
RO-F-17 & Il sistema deve permettere l'applicazione del grassetto al testo tramite toolbar. & UC-1.8.2 \\
\hline
RO-F-18 & Il sistema deve permettere l'applicazione del corsivo al testo tramite toolbar. & UC-1.8.3 \\
\hline
RO-F-19 & Il sistema deve permettere l'applicazione della sottolineatura al testo tramite toolbar. & UC-1.8.4 \\
\hline
RO-F-20 & Il sistema deve permettere di creare intestazioni tramite toolbar. & UC-1.8.5 \\
\hline
RO-F-21 & Il sistema deve permettere l'inserimento di elementi di struttura (tabelle, separatori orizzontali) tramite toolbar. & UC-1.8.6 \\
\hline
RO-F-22 & Il sistema deve permettere l'inserimento di liste numerate tramite toolbar. & UC-1.8.7 \\
\hline
RO-F-23 & Il sistema deve permettere l'inserimento di elenchi puntati tramite toolbar. & UC-1.8.7 \\
\hline
RO-F-24 & Il sistema deve permettere l'inserimento di link e riferimenti tramite toolbar. & UC-1.8.8 \\
\hline
...
\hline
RO-F-26 & L'utente può annullare una formattazione di una porzione di testo selezionato riapplicandola dalla toolbar. & UC-1.8.1 \\
\hline
RO-F-27 & L'utente può estendere la formattazione a una porzione di testo parzialmente formattato riapplicandola dalla toolbar. & UC-1.8.0.1 \\
\hline
RO-F-28 & L'utente può creare sottoliste attivando la creazione di una lista dalla toolbar quando il cursore è già all'interno di una lista. & UC-1.8.7.1 \\
\hline
RO-F-29 & L'utente può scegliere una combinazione di formattazioni dalla toolbar quando non è presente testo selezionato, per applicarle al testo che scriverà successivamente. & UC-1.9.* \\
\hline
...
\hline
RO-F-59 & Il sistema permette l'invio di un prompt scritto dall'utente per la generazione di testo tramite agente esterno. & UC-2.7 \\
\hline
RO-F-60 & Il sistema permette la modifica del prompt da inviare all'agente esterno prima dell'invio. & UC-2.7 \\
\hline
RO-F-61 & L'agente esterno può generare testo basandosi su un prompt fornitogli dall'utente. & UC-2.7.* \\
\hline
RO-F-62 & Se il prompt fornito dall'utente è vuoto o incompleto, il sistema notifica l'utente e impedisce l'invio. & UC-2.7.1 \\
\hline
RO-F-63 & L'utente può caricare un file dal proprio dispositivo all'interno dell'editor. & UC-3.* \\
\hline
RO-F-64 & Il sistema deve supportare il caricamento di file in formato Markdown (\texttt{.md}, \texttt{.markdown}) e testo semplice (\texttt{.txt}). & UC-3.0, UC-3.0.1, UC-3.0.2 \\
\hline
...
\hline
RO-F-73 & Il sistema deve essere in grado di convertire il documento da Markdown al formato selezionato dall'utente. & UC-4.0 \\
\hline
...
\hline
RO-F-77 & Il sistema deve consentire l'esportazione del documento in formato Markdown (\texttt{.md}). & UC-4.0.2 \\
\hline
...
\hline
RO-F-79 & Il sistema deve permettere di collegare e inviare il documento a un dispositivo di stampa. & UC-4.2 \\
\hline
RO-F-80 & Il sistema deve informare l'utente con un messaggio in caso di errore durante l'invio del documento alla stampante. & UC-4.2 \\
\hline
...
\hline
RD-F-7 & Il sistema deve offrire più opzioni per le operazioni comuni sui file come carica nuovo, stampa, chiudi & UC-3.0, UC-3.3, UC-4.0, UC-4.2\\
\hline
...
\hline
RD-F-15 & Il sistema deve permettere all'utente di scegliere se stampare tutto il documento o un range di pagine selezionate & UC-4.2, UC-4.2.1\\
\hline
...
\hline
ROP-F-24 & Il sistema deve impedire la registrazione in caso ci sia uno username uguale nel database & UC-5.1 \\
\hline
ROP-F-25 & Il sistema deve salvare nel database le password criptate in fase di registrazione & UC-5.1 \\
\hline
ROP-F-26 & Il sistema deve permettere ad un utente autenticato di poter eliminare il proprio account & UC-5.2 \\
\hline
ROP-F-27 & Il sistema in caso di eliminazione account deve eliminare i dati dal database & UC-5.2 \\
\hline
...
\hline
ROP-F-31 & Il sistema deve permettere ad un utente di vedere i file che ha salvato & UC-5.3 \\
\hline
ROP-F-32 & Il sistema deve permettere ad un utente autenticato di selezionare un file dall'elenco di quelli salvati e aprirlo & UC-5.3 \\
\hline
ROP-F-33 & Il sistema deve permettere all'utente di annullare l'operazione di apertura file, lasciando l'editor nel suo stato precedente. & UC-5.3.1 \\
\hline
ROP-F-34 & Il sistema deve mostrare i file salvati in ordine cronologico per data di ultima modifica. & UC-5.3 \\
\hline
ROP-F-35 & L'accesso ai file salvati nel database è consentito solo ad utenti autenticati. & UC-5.3 \\
\hline
ROP-F-36 & Il sistema deve permettere il salvataggio di un file in un sistema di persistenza non locale & UC-5.4 \\
\hline
ROP-F-37 & Il sistema deve notificare l'utente in caso di fallimento del salvataggio nel database. & UC-5.4 \\
\hline
ROP-F-38 & Il sistema deve associare ogni file al suo creatore & UC-5.4 \\
\hline
ROP-F-39 & Il sistema deve impedire il salvataggio di file nel database da parte di utenti non autenticati. & UC-5.4, UC-5.4.2 \\
\hline
ROP-F-40 & Il sistema deve impedire il salvataggio di un documento vuoto nel database. & UC-5.4.1 \\
\hline
ROP-F-41 & Un utente non autenticato che prova a salvare un file viene riportato alla pagina di login & UC-5.4.2 \\
\hline
ROP-F-42 & L'utente può effettuare il logout in ogni momento e terminare la propria sessione & UC-5.5 \\
\hline
\addlinespace[1em]
...
I requisiti di vincolo\textsubscript{G} rappresentano le limitazioni entro le quali il progetto deve essere sviluppato. A differenza dei requisiti funzionali, questi non derivano da scelte progettuali del team, ma sono imposizioni esterne dettate dal committente, da normative o da limiti tecnologici. Questi comprendono i vincoli tecnologici, architetturali e organizzativi.
...
\hline
RO-V-3 & Il formato di input e di storage del testo deve essere Markdown, in conformità allo standard CommonMark. & Capitolato \\
\hline
RO-V-4 & Il sistema deve integrarsi con un servizio LLM tramite API REST compatibili con lo standard OpenAI. & Capitolato \\
\hline
RO-V-5 & Il sistema deve essere sviluppato rispettando le Norme 
di Progetto del gruppo ByteMe. & Norme di Progetto v1.0.0 \\
\hline
RO-V-6 & Il sistema deve essere verificato secondo il Piano di Qualifica prima di ogni rilascio. & Piano di Qualifica v1.0.0 \\
\hline
\addlinespace[1em]
...
I requisiti di qualità\textsubscript{G} specificano le caratteristiche 
...
\textbf{Requisito} & \textbf{Descrizione} & \textbf{Fonte} \\
\hline
RO-Q-1 & Il tempo di risposta dell'interfaccia utente (aggiornamento anteprima, applicazione formattazioni) deve essere inferiore a 500 ms nel 95\% dei casi. & PdQ – Response Time, soglia accettabile \\
\hline
RO-Q-2 & Il tempo medio di generazione di una risposta da parte del servizio LLM non deve superare i 10 secondi. & PdQ – AI Processing Time, soglia accettabile \\
\hline
RO-Q-3 & Il tasso di errori nelle operazioni principali (salvataggio, apertura file, chiamate AI) non deve superare l'1\%. & PdQ – Error Rate, soglia accettabile \\
\hline
RO-Q-4 & La percentuale di codice coperta dai test automatici (test coverage) deve essere almeno il 70\%. & PdQ – Test Coverage, soglia accettabile \\
\hline
...
\hline
UC-1.8.0 & Modifica testo selezionato tramite toolbar & RO-F-16 \\
\hline
UC-1.8.0.1 & Testo già parzialmente formattato & RO-F-27 \\
\hline
UC-1.8.1 & Rimozione formattazione tramite toolbar & RO-F-26 \\
\hline
...
\hline
UC-2.7 & Generazione di testo (Distant Writing) & RO-F-59, RO-F-60, RO-F-61, ROP-F-11 \\
\hline
UC-2.7.1 & Gestione prompt incompleto & RO-F-62 \\
\hline
...
\hline
UC-3.0.1 & Caricamento file Markdown & RO-F-64 \\
\hline
...
\hline
UC-4.2 & Stampare il documento & RO-F-75, RO-F-79, RO-F-80, RD-F-7 \\
\hline
UC-4.2.1 & Stampare pagine selezionate & RD-F-15 \\
\hline
...
\hline
UC-5.3 & Apertura di un file salvato & ROP-F-31, ROP-F-32, ROP-F-34, ROP-F-35 \\
\hline
UC-5.3.1 & Gestione annullamento apertura & ROP-F-33 \\
\hline
UC-5.4 & Salvataggio file nel database & ROP-F-36, ROP-F-37, ROP-F-38, ROP-F-39 \\
\hline
UC-5.4.1 & Gestione salvataggio nota vuota & ROP-F-40 \\
\hline
UC-5.4.2 & Gestione salvataggio senza autenticazione & ROP-F-39, ROP-F-41 \\
\hline
UC-5.5 & Logout & ROP-F-42 \\
\hline
\addlinespace[1em]
